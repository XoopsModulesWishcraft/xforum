<?php
// $Id: viewtopic.php,v 4.03 2008/06/05 15:35:59 wishcraft Exp $
//  ------------------------------------------------------------------------ //
...
$xoops_module_header .= '<script src="'.XOOPS_URL.'/Frameworks/textsanitizer/xoops.js" type="text/javascript"></script>';
	...
	if ($xoopsModuleConfig['htaccess'])
	{
		...
		if (strlen($xforum_name)>0&&strlen($topic_title)>0){
			...
			//echo $sql;
			$sql = "SELECT topic_id FROM ".$xoopsDB->prefix('xf_topics')." WHERE topic_title LIKE '".str_replace(array('\\_',"-"),"_",addslashes($topic_title))."' and forum_id = '$xforum_id'";
			$ret = $xoopsDB->queryF($sql);
			$rt = $xoopsDB->fetchArray($ret);
			$topic_id = $rt['topic_id'];
		
			$sql = "SELECT post_id FROM ".$xoopsDB->prefix('xf_posts')." WHERE topic_id = '$topic_id' and forum_id = '$xforum_id' order by post_time asc limit 1";
			$ret = $xoopsDB->queryF($sql);
			$rt = $xoopsDB->fetchArray($ret);
			$post_id = $rt['post_id'];	
			...
			$xforum_id = !empty($_GET['forum']) ? intval($_GET['forum']) : $xforum_id;
			...
			$xforum_id = !empty($_GET['forum']) ? intval($_GET['forum']) : 0;
			...
			header( "Location: ".XOOPS_URL."/forums/".xoops_sef($xforum_name)."/".xoops_sef($topic_title)."/$xforum_id,$topic_id,$post_id,$start,$type,$mode,$move");
		}
	} else {
		$topic_id = isset($_GET['topic_id']) ? intval($_GET['topic_id']) : 0;
		$post_id = !empty($_GET['post_id']) ? intval($_GET['post_id']) : 0;
...
if ( !$topic_id && !$post_id ) {
	$redirect = empty($xforum_id)?"index.php":'viewforum.php?forum='.$xforum_id;
...
if ( !empty($post_id) ) {
    ...
    $xforumtopic =& $topic_handler->getByMove($topic_id, ($move == "prev")?-1:1, $xforum_id);
...
if ( !is_object($xforumtopic) || !$topic_id = $xforumtopic->getVar('topic_id') ) {
    redirect_header('viewforum.php?forum='.$xforum_id, 2, _MD_ERRORTOPIC);
}
$xforum_id = $xforumtopic->getVar('forum_id');
$xforum_handler =& xoops_getmodulehandler('forum', 'xforum');
$viewtopic_forum =& $xforum_handler->get($xforum_id);

$isadmin = xforum_isAdmin($viewtopic_forum);

if(!$isadmin && $xforumtopic->getVar('approved')<0 ){
    redirect_header("viewforum.php?forum=".$xforum_id,2,_MD_NORIGHTTOVIEW);
    exit();
}
...
$permission_set = $perm->getPermissions('forum', $xforum_id);
...
$viewmode_cookie = xforum_getcookie("V");
if(isset($_GET['viewmode']) && in_array($_GET['viewmode'], $valid_modes)) {
	xforum_setcookie("V", $_GET['viewmode'], $xforumCookie['expire']);
}
...
//$topic_lastread = xforum_getcookie('LT',true);
//if ( empty($topic_lastread[$topic_id]) ) {
    $xforumtopic->incrementCounter();
//}
/*
$topic_lastread[$topic_id] = time();
xforum_setcookie("LT", $topic_lastread);
*/
xforum_setRead("topic", $topic_id, $xforumtopic->getVar("topic_last_post_id"));
...
if($xoopsTpl->xoops_canUpdateFromFile() && is_dir(XOOPS_THEME_PATH."/".$xoopsConfig['theme_set']."/templates/".$xoopsModule->getVar("dirname"))){
	$xoopsTpl->assign('xforum_template_path', XOOPS_THEME_PATH."/".$xoopsConfig['theme_set']."/templates/".$xoopsModule->getVar("dirname"));
}else{
	$xoopsTpl->assign('xforum_template_path', XOOPS_ROOT_PATH."/modules/".$xoopsModule->getVar("dirname")."/templates");
}
...
$xoopsTpl->assign(array(
	'topic_title' 		=> '<a href="'.$xforumUrl['root'].'/viewtopic.php?viewmode='.$viewmode.'&amp;type='.$type.'&amp;topic_id='.$topic_id.'&amp;forum='.$xforum_id.'">'. $topic_prefix.$xforumtopic->getVar('topic_title').'</a>', 
...
$xoopsTpl->assign('forum_id', $xforum_id);
...
$t_new = xforum_displayImage($xforumImage['t_new'],_MD_POSTNEW);
$t_reply = xforum_displayImage($xforumImage['t_reply'],_MD_REPLY);

if ($topic_handler->getPermission($viewtopic_forum, $xforumtopic->getVar('topic_status'), "post")){
    $xoopsTpl->assign('forum_post_or_register', "<a href=\"newtopic.php?forum=".$xforum_id."\">".$t_new."</a>");
...
if ($topic_handler->getPermission($viewtopic_forum, $xforumtopic->getVar('topic_status'), "reply")){
    $xoopsTpl->assign('forum_reply', "<a href=\"reply.php?forum=".$xforum_id."&amp;topic_id=".$topic_id."\">".$t_reply."</a>");
}
...
$xoopsTpl->assign('topic_print_link', "print.php?form=1&amp;topic_id=$topic_id&amp;forum=".$xforum_id."&amp;order=$order&amp;start=$start");
...
$admin_actions['merge'] = array(
	"link" => $xforumUrl['root'].'/topicmanager.php?mode=merge&amp;topic_id='.$topic_id.'&amp;forum='.$xforum_id,
	"name" => _MD_MERGETOPIC,
	"image" => $ad_merge);
$admin_actions['move'] = array(
	"link" => $xforumUrl['root'].'/topicmanager.php?mode=move&amp;topic_id='.$topic_id.'&amp;forum='.$xforum_id,
	"name" => _MD_MOVETOPIC,
	"image" => $ad_move);
$admin_actions['delete'] = array(
	"link" => $xforumUrl['root'].'/topicmanager.php?mode=delete&amp;topic_id='.$topic_id.'&amp;forum='.$xforum_id,
	"name" => _MD_DELETETOPIC,
	"image" => $ad_delete);
if ( !$xforumtopic->getVar('topic_status') ){
    $admin_actions['lock'] = array(
    	"link"=>$xforumUrl['root'].'/topicmanager.php?mode=lock&amp;topic_id='.$topic_id.'&amp;forum='.$xforum_id,
    	"image"=>$ad_lock,
    	"name"=>_MD_LOCKTOPIC);
}else{
    $admin_actions['unlock'] = array(
    	"link"=>$xforumUrl['root'].'/topicmanager.php?mode=unlock&amp;topic_id='.$topic_id.'&amp;forum='.$xforum_id,
    	"image"=>$ad_unlock,
    	"name"=>_MD_UNLOCKTOPIC);
}
if ( !$xforumtopic->getVar('topic_sticky') ){
    $admin_actions['sticky'] = array(
    	"link"=>$xforumUrl['root'].'/topicmanager.php?mode=sticky&amp;topic_id='.$topic_id.'&amp;forum='.$xforum_id,
    	"image"=>$ad_sticky,
    	"name"=>_MD_STICKYTOPIC);
}else{
    $admin_actions['unsticky'] = array(
    	"link"=>$xforumUrl['root'].'/topicmanager.php?mode=unsticky&amp;topic_id='.$topic_id.'&amp;forum='.$xforum_id,
    	"image"=>$ad_unsticky,
    	"name"=>_MD_UNSTICKYTOPIC);
}
if ( !$xforumtopic->getVar('topic_digest') ){
    $admin_actions['digest'] = array(
    	"link"=>$xforumUrl['root'].'/topicmanager.php?mode=digest&amp;topic_id='.$topic_id.'&amp;forum='.$xforum_id,
    	"image"=>$ad_digest,
    	"name"=>_MD_DIGESTTOPIC);
}else{
    $admin_actions['undigest'] = array(
    	"link"=>$xforumUrl['root'].'/topicmanager.php?mode=undigest&amp;topic_id='.$topic_id.'&amp;forum='.$xforum_id,
    	"image"=>$ad_undigest,
...
if ($topic_handler->getPermission($viewtopic_forum, $xforumtopic->getVar('topic_status'), "addpoll")
	...
	if(!$xforumtopic->getVar('topic_haspoll')){	
		if( is_object($xoopsUser) && $xoopsUser->getVar("uid")==$xforumtopic->getVar("topic_poster") ){
			...
			$xoopsTpl->assign('forum_addpoll', "<a href=\"polls.php?op=add&amp;forum=".$xforum_id."&amp;topic_id=".$topic_id."\">".$t_poll."</a>&nbsp;");
		}
	}elseif($isadmin
	    ...
	    $adminpoll_actions['editpoll'] = array(
	    	"link" => $xforumUrl['root'].'/polls.php?op=edit&amp;poll_id='.$xforumtopic->getVar('poll_id').'&amp;topic_id='.$topic_id.'&amp;forum='.$xforum_id,
	    	"image" => $poll_edit,
	    	"name" => _MD_EDITPOLL);
	    $adminpoll_actions['deletepoll'] = array(
	    	"link" => $xforumUrl['root'].'/polls.php?op=delete&amp;poll_id='.$xforumtopic->getVar('poll_id').'&amp;topic_id='.$topic_id.'&amp;forum='.$xforum_id,
	    	"image" => $poll_delete,
	    	"name" => _MD_DELETEPOLL);
	    $adminpoll_actions['restartpoll'] = array(
	    	"link" => $xforumUrl['root'].'/polls.php?op=restart&amp;poll_id='.$xforumtopic->getVar('poll_id').'&amp;topic_id='.$topic_id.'&amp;forum='.$xforum_id,
	    	"image" => $poll_restart,
...
$xoopsTpl->assign('p_up',xforum_displayImage($xforumImage['p_up'],_MD_TOP));
...
$xoopsTpl->assign('threaded',xforum_displayImage($xforumImage['threaded']));
$xoopsTpl->assign('flat',xforum_displayImage($xforumImage['flat']));
$xoopsTpl->assign('left',xforum_displayImage($xforumImage['left']));
$xoopsTpl->assign('right',xforum_displayImage($xforumImage['right']));
$xoopsTpl->assign('down',xforum_displayImage($xforumImage['doubledown']));
$xoopsTpl->assign('down2',xforum_displayImage($xforumImage['down']));
$xoopsTpl->assign('up',xforum_displayImage($xforumImage['up']));
$xoopsTpl->assign('printer',xforum_displayImage($xforumImage['printer']));
$xoopsTpl->assign('personal',xforum_displayImage($xforumImage['personal']));
$xoopsTpl->assign('post_content',xforum_displayImage($xforumImage['post_content']));
...
if(!empty($xoopsModuleConfig['rating_enabled'])){
	...
	if ( $rating < 1 ) {
		$rating_img = xforum_displayImage($xforumImage['blank']);
	}else{
		$rating_img  = xforum_displayImage($xforumImage['rate'.$rating]);
	}
	$xoopsTpl->assign('rating_img',$rating_img);
	$xoopsTpl->assign('rate1',xforum_displayImage($xforumImage['rate1'],_MD_RATE1));
	$xoopsTpl->assign('rate2',xforum_displayImage($xforumImage['rate2'],_MD_RATE2));
	$xoopsTpl->assign('rate3',xforum_displayImage($xforumImage['rate3'],_MD_RATE3));
	$xoopsTpl->assign('rate4',xforum_displayImage($xforumImage['rate4'],_MD_RATE4));
	$xoopsTpl->assign('rate5',xforum_displayImage($xforumImage['rate5'],_MD_RATE5));
}

// create jump box
if(!empty($xoopsModuleConfig['show_jump'])){
	$xoopsTpl->assign('forum_jumpbox', xforum_make_jumpbox($xforum_id));
}
...
if($viewmode=="thread"){
	$viewmode_options[]= array("link"=>$xforumUrl['root']."/viewtopic.php?viewmode=flat&amp;topic_id=".$topic_id."&amp;forum=".$xforum_id, "title"=>_FLAT);
	$viewmode_options[]= array("link"=>$xforumUrl['root']."/viewtopic.php?viewmode=compact&amp;topic_id=".$topic_id."&amp;forum=".$xforum_id,	"title"=>_MD_COMPACT);
}elseif($viewmode=="compact"){
	$viewmode_options[]= array("link"=>$xforumUrl['root']."/viewtopic.php?viewmode=thread&amp;topic_id=".$topic_id."&amp;forum=".$xforum_id,"title"=>_THREADED);
	$viewmode_options[]= array("link"=>$xforumUrl['root']."/viewtopic.php?viewmode=flat&amp;order=".$order_current."&amp;topic_id=".$topic_id."&amp;forum=".$xforum_id,	"title"=>_FLAT);
	if ($order == 'DESC') {
		$viewmode_options[]= array("link"=>$xforumUrl['root']."/viewtopic.php?viewmode=compact&amp;order=ASC&amp;topic_id=".$topic_id."&amp;forum=".$xforum_id,"title"=>_OLDESTFIRST);
	} else {
		$viewmode_options[]= array("link"=>$xforumUrl['root']."/viewtopic.php?viewmode=compact&amp;order=DESC&amp;topic_id=".$topic_id."&amp;forum=".$xforum_id,"title"=>_NEWESTFIRST);
	}
}else{
	$viewmode_options[]= array("link"=>$xforumUrl['root']."/viewtopic.php?viewmode=thread&amp;topic_id=".$topic_id."&amp;forum=".$xforum_id,"title"=>_THREADED);
	$viewmode_options[]= array("link"=>$xforumUrl['root']."/viewtopic.php?viewmode=compact&amp;order=".$order_current."&amp;topic_id=".$topic_id."&amp;forum=".$xforum_id, "title"=>_MD_COMPACT);
	if ($order == 'DESC') {
		$viewmode_options[]= array("link"=>$xforumUrl['root']."/viewtopic.php?viewmode=flat&amp;order=ASC&amp;type=$type&amp;topic_id=".$topic_id."&amp;forum=".$xforum_id, "title"=>_OLDESTFIRST);
	} else {
		$viewmode_options[]= array("link"=>$xforumUrl['root']."/viewtopic.php?viewmode=flat&amp;order=DESC&amp;type=$type&amp;topic_id=".$topic_id."&amp;forum=".$xforum_id, "title"=>_NEWESTFIRST);
	}
}
switch($type){
	case 'active':
...
if( !empty($xoopsModuleConfig['quickreply_enabled'])
	...
	$xforum_form = new XoopsThemeForm(_MD_POSTREPLY, 'quick_reply', "post.php", 'post', true);
	...
	if(!is_object($xoopsUser)){
		...
		$xforum_form->addElement($user_tray, '');
	}
	...
	$xforum_form->addElement($editor_object, true);

	$xforum_form->addElement(new XoopsFormHidden('dohtml', 0));
	$xforum_form->addElement(new XoopsFormHidden('dosmiley', 1));
	$xforum_form->addElement(new XoopsFormHidden('doxcode', 1));
	$xforum_form->addElement(new XoopsFormHidden('dobr', 1));
	$xforum_form->addElement(new XoopsFormHidden('attachsig', 1));

	$xforum_form->addElement(new XoopsFormHidden('isreply', 1));
	
	$xforum_form->addElement(new XoopsFormHidden('subject', _MD_RE.': '.$xforumtopic->getVar('topic_title', 'e')));
	$xforum_form->addElement(new XoopsFormHidden('pid', empty($post_id)?$topic_handler->getTopPostId($topic_id):$post_id));
	$xforum_form->addElement(new XoopsFormHidden('topic_id', $topic_id));
	$xforum_form->addElement(new XoopsFormHidden('forum', $xforum_id));
	$xforum_form->addElement(new XoopsFormHidden('viewmode', $viewmode));
	$xforum_form->addElement(new XoopsFormHidden('order', $order));
	$xforum_form->addElement(new XoopsFormHidden('start', $start));

	// backward compatible
	if(!class_exists("XoopsSecurity")){
		$post_valid = 1;
		$_SESSION['submit_token'] = $post_valid;
		$xforum_form->addElement(new XoopsFormHidden('post_valid', $post_valid));
	}

	$xforum_form->addElement(new XoopsFormHidden('notify', -1));
	$xforum_form->addElement(new XoopsFormHidden('contents_submit', 1));
	...
	$xforum_form->addElement($button_tray);

	$toggles = xforum_getcookie('G', true);
	$display = (in_array('qr', $toggles)) ? 'none;' : 'block;';
    $xoopsTpl->assign('quickreply', array( 'show' => 1, 'display'=>$display, 'icon'=>xforum_displayImage($xforumImage['t_qr']), 'form' => $xforum_form->render()));
	unset($xforum_form);
}else{
