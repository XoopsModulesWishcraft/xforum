<?php
// $Id: action.post.php,v 4.03 2008/06/05 16:23:23 wishcraft Exp $
//  ------------------------------------------------------------------------ //
...
switch($op){
	case "restore":
		...
		foreach(array_keys($xforums) as $xforum){
			$xforum_handler->synchronization($xforum);
		}
		break;
	case "approve":
		$post_id = array_values($post_id);
		...
		$xforum_list =& $xforum_handler->getList($criteria_forum);
		...
		foreach($post_id as $post){
		    ...
		    $tags['FORUM_NAME'] = $xforum_list[$posts_obj[$post]->getVar('forum_id')];
	...
	case "split":
        ...
        $xforum_id = $post_obj->getVar("forum_id");
		$topic_handler->synchronization($topic_id);
		$topic_handler->synchronization($new_topic_id);
        $sql = sprintf("UPDATE %s SET forum_topics = forum_topics+1 WHERE forum_id = %u", $xoopsDB->prefix("xf_forums"), $xforum_id);
