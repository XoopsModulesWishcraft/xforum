<?php
// $Id: post.php,v 4.03 2008/06/05 15:35:59 wishcraft Exp $
//  ------------------------------------------------------------------------ //
...
if ( !empty($_POST['contents_submit']) ) {
	...
	$isadmin = xforum_isAdmin($forum_obj);

	$time_valid = true;
	if( !$isadmin && !empty($xoopsModuleConfig['post_timelimit']) ){
    	$last_post = xforum_getsession('LP'); // using session might be more secure ...
...
if ( !empty($_POST['contents_submit']) ) {
    ...
    if ( !empty($isedit) && $post_id>0) {
        ...
        $forumpost->setVar('poster_ip', xforum_getIP());
	...
	if($dohtml && !xforum_isAdmin($forum_obj) ) {
		//$message=xforum_textFilter($message);
	}
    ...
    )
    {
        ...
        {
	        $prefix = is_object($xoopsUser)?strval($xoopsUser->uid()).'_':'xforum_';
	...
	xforum_setsession("LP", time()); // Recording last post time


    if(xforum_checkSubjectPrefixPermission($forum_obj) && !empty($_POST['subject_pre'])){
    ...
    if($approved && !empty($xoopsModuleConfig['notification_enabled']) && !empty($isnew)){
	    ...
	    $forum_info = xforum_notify_iteminfo ('forum', $forum_obj->getVar('forum_id'));
    ...
    if($approved){
		if(!empty($xoopsModuleConfig['cache_enabled'])){
			xforum_setsession("t".$forumpost->getVar("topic_id"), null);
		}
...
if ( !empty($_POST['contents_preview']) || !empty($_GET['contents_preview']) ) {
	...
	if($dohtml && !xforum_isAdmin($forum_obj) ) {
		//$p_message = xforum_textFilter($p_message);
	}
