...
{
    global $myts, $xoopsDB, $xoopsModule, $xforum_handler;

    //$xforum_handler = &xoops_getmodulehandler('forum', 'xforum');
    if (!is_object($ff)) {
        $ff =& $xforum_handler->create();
        $new = true;
        $xforum = 0;
    } else {
    ...
    if ($parent_forum > 0) {
        $pf =& $xforum_handler->get($parent_forum);
    }
...
switch ($op) {
    case 'moveforum':
        ...
        if(!empty($_POST['dest'])) {
            if (!empty($_GET['forum'])) $xforum_id = intval($_GET['forum']);
            if (!empty($_POST['forum'])) $xforum_id = intval($_POST['forum']);

            $dest = $_POST['dest'];
            if($dest{0}=="f"){
	            $pid = substr($dest, 1);
            	$xforum = &$xforum_handler->get(intval($pid));
            ...
            $sql_move = "UPDATE " . $xoopsDB->prefix('xf_forums') . " SET " . $value . " WHERE forum_id=$xforum_id";
            if ($result = $xoopsDB->queryF($sql_move)){
                $bMoved = 1;
            	$sql = "UPDATE " . $xoopsDB->prefix('xf_forums') . " SET parent_forum = 0 WHERE parent_forum=$xforum_id";
            ...
            if (!empty($_POST['forum'])) $xforum_id = intval($_POST['forum']);
            if (!empty($_GET['forum'])) $xforum_id = intval($_GET['forum']);
            //$xforum = &$xforum_handler->get($xforum_id);
		    ...
		    $xforums = $xforum_handler->getForumsByCategory(array_keys($categories), '', false);
            ...
            echo '<input type="hidden" name="forum" value=' . $xforum_id . ' />';
    ...
    case 'mergeforum':
        ...
        if (!empty($_POST['dest_forum'])) {
            if (isset($_GET['forum'])) $xforum_id = intval($_GET['forum']);
            if (isset($_POST['forum'])) $xforum_id = intval($_POST['forum']);
            ...
            $sql = "SELECT COUNT(*) AS count FROM " . $xoopsDB->prefix('xf_posts') . " WHERE WHERE forum_id=$xforum_id";
            $result = $xoopsDB->query($sql);
            list($post_count) = $xoopsDB->fetchArray($result);
            $sql = "SELECT COUNT(*) AS count FROM " . $xoopsDB->prefix('xf_topics') . " WHERE WHERE forum_id=$xforum_id";
            $result = $xoopsDB->query($sql);
            list($topic_count) = $xoopsDB->fetchArray($result);

            $xforum = &$xforum_handler->get($xforum_id);
            $xforum_handler->delete($xforum);
			...
			//$category_handler =& xoops_getmodulehandler('category', 'xforum');
		    $xforums = $xforum_handler->getForumsByCategory(0, '', false);
    ...
    case "del":
        ...
        if (isset($_POST['confirm']) != 1) {
            ...
            $ff = &$xforum_handler->get($_POST['forum']);
            $xforum_handler->delete($ff);
    ...
    case 'manage':
		...
		foreach (array_keys($categories) as $c) {
            ...
            $cat_edit_link = "<a href=\"admin_cat_manager.php?op=mod&amp;cat_id=" . $category->getVar('cat_id') . "\">".xforum_displayImage($xforumImage['edit'], _EDIT)."</a>";
            $cat_del_link = "<a href=\"admin_cat_manager.php?op=del&amp;cat_id=" . $category->getVar('cat_id') . "\">".xforum_displayImage($xforumImage['delete'], _DELETE)."</a>";
            $xforum_add_link = "<a href=\"admin_forum_manager.php?op=addforum&amp;cat_id=" . $category->getVar('cat_id') . "\">".xforum_displayImage($xforumImage['new_forum'])."</a>";
            ...
            $echo .= "<td align='center'>" . $xforum_add_link . "</td>";
            $echo .= "<td></td>";
            $echo .= "<td></td>";
            $echo .= "</tr>";
			...
			foreach(array_keys($xforums[$c]) as $f){
                ...
                $f_edit_link = "<a href=\"admin_forum_manager.php?op=mod&amp;forum=" . $f . "\">".xforum_displayImage($xforumImage['edit'])."</a>";
                $f_del_link = "<a href=\"admin_forum_manager.php?op=del&amp;forum=" . $f . "\">".xforum_displayImage($xforumImage['delete'])."</a>";
                $sf_add_link = "<a href=\"admin_forum_manager.php?op=addsubforum&amp;cat_id=" . $c . "&parent_forum=" . $f . "\">".xforum_displayImage($xforumImage['new_subforum'])."</a>";
                $f_move_link = "<a href=\"admin_forum_manager.php?op=moveforum&amp;forum=" . $f . "\">".xforum_displayImage($xforumImage['move_topic'])."</a>";
                $f_merge_link = "<a href=\"admin_forum_manager.php?op=mergeforum&amp;forum=" . $f . "\">".xforum_displayImage($xforumImage['move_topic'])."</a>";
				...
				foreach(array_keys($xforums[$c][$f]["sub"]) as $s){
                    ...
                    $f_edit_link = "<a href=\"admin_forum_manager.php?op=mod&amp;forum=" . $s . "\">".xforum_displayImage($xforumImage['edit'])."</a>";
                    $f_del_link = "<a href=\"admin_forum_manager.php?op=del&amp;forum=" . $s . "\">".xforum_displayImage($xforumImage['delete'])."</a>";
                    $sf_add_link = "";
                    $f_move_link = "<a href=\"admin_forum_manager.php?op=moveforum&amp;forum=" . $s . "\">".xforum_displayImage($xforumImage['move_topic'])."</a>";
                    $f_merge_link = "<a href=\"admin_forum_manager.php?op=mergeforum&amp;forum=" . $s . "\">".xforum_displayImage($xforumImage['move_topic'])."</a>";
                    $echo .= "<tr class='odd' align='left'><td></td>";
                    $echo .= "<td><strong>" . $f_link . "</strong></td>";
                    $echo .= "<td align='center'>" . $f_edit_link . "</td>";
                    $echo .= "<td align='center'>" . $f_del_link . "</td>";
