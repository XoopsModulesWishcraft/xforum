...
function xforum_admin_getPathStatus($path)
{
...
function xforum_admin_mkdir($target, $mode=0777)
{
	// http://www.php.net/manual/en/function.mkdir.php
	return is_dir($target) or ( xforum_admin_mkdir(dirname($target), $mode) and mkdir($target, $mode) );
}

function xforum_admin_chmod($target, $mode = 0777)
{
	return @chmod($target, $mode);
}

function xforum_getImageLibs()
{
...
switch ($op) {
    ...
    case "setperm":
		if (isset($_GET['path'])) $path = $_GET['path'];
        $res = xforum_admin_chmod($path, 0777);
    ...
    default:
        ...
        $path_status = xforum_admin_getPathStatus($attach_path);
        echo $attach_path . ' ( ' . $path_status . ' )';

        echo "<br />" . _AM_XFORUM_THUMBPATH . ": ";
        $thumb_path = $attach_path . 'thumbs/'; // be careful
        $path_status = xforum_admin_getPathStatus($thumb_path);
