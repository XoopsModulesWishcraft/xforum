<?php
// $Id: search.inc.php,v 4.03 2008/06/05 15:35:33 wishcraft Exp $
//  ------------------------------------------------------------------------ //
...
function &xforum_search($queryarray, $andor, $limit, $offset, $userid, $xforums = 0, $sortby = 0, $searchin = "both", $subquery = "")
{
	...
	if(!isset($allowedForums[$uid])){
		...
		if (is_array($xforums) && count($xforums) > 0) {
			...
			foreach($xforums as $xforumid){
				$_forum = $xforum_handler->get($xforumid);
				if($xforum_handler->getPermission($_forum)) {
		...
		elseif (is_numeric($xforums) && $xforums > 0) {
			...
			$xforums = $xforum_handler->getForums();
