...
if(!defined("xforum_FUNCTIONS_IMAGE")):
define("xforum_FUNCTIONS_IMAGE", true);

function xforum_attachmentImage($source)
{
	...
	if($xoopsModuleConfig['max_img_width']>0){
		...
		if(!file_exists($thumb_path.'/'.$source) && $imginfo[0]>$xoopsModuleConfig['max_img_width']){
			xforum_createThumbnail($source, $xoopsModuleConfig['max_img_width']);
		}
	}


	if(file_exists($thumb)){
		$attachmentImage  = '<a href="'.$image_url.'" title="'.$source.' '.$img_info.'" target="xforum_image">';
...
function xforum_createThumbnail($source, $thumb_width)
{
	global $xoopsModuleConfig;
