<?php
// $Id: vars.php,v 4.03 2008/06/05 15:35:33 wishcraft Exp $
//  ------------------------------------------------------------------------ //
 ...
 *
 **/
define('xforum_CONSTANTS',1);
define('xforum_READ', 1);
define('xforum_UNREAD', 2);
define('xforum_UNREPLIED', 3);
define('xforum_DIGEST', 4);
define('xforum_DELETEONE', 1);
define('xforum_DELETEALL', 2);
...
$xforumCookie['domain'] = "";
$xforumCookie['path'] = "/";
$xforumCookie['secure'] = false;
$xforumCookie['expire'] = time() + 3600 * 24 * 30; // one month
$xforumCookie['prefix'] = '';
...
if(empty($xforumCookie['prefix'])){
	...
	$xforumCookie['prefix'] = $cookie_prefix."_".$xoopsModule->dirname().$cookie_userid."_";
}
...
//$last_visit = xforum_getcookie("LVT");
$last_visit = xforum_getsession("LV");
$last_visit = ($last_visit)?$last_visit:xforum_getcookie("LV");
$last_visit = ($last_visit)?$last_visit:time();


// update LastVisit cookie.
xforum_setcookie("LV", time(), $xforumCookie['expire']); // set cookie life time to one month
//xforum_setcookie("LVT", $last_visit);
xforum_setsession("LV", $last_visit);
...
if( is_object($GLOBALS["xoopsModule"]) && "xforum" == $GLOBALS["xoopsModule"]->getVar("dirname", "n") ) {
	$GLOBALS["xoopsModuleConfig"] = xforum_load_config();
}
