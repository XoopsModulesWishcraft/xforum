...
function xforum_getGroupsByUser($uid)
{
	global $xoopsDB;	
    $ret = array();
...
function xforum_getrank($rank_id =0, $posts = 0)
{
	static $ranks;
...
class User extends XoopsObject
{
    ...
    function getInfo()
    {
		...
		$userinfo["regdate"] = xforum_formatTimestamp($user->getVar('user_regdate'), 'reg');
		...
		$rank = xforum_getrank($user->getVar("rank"), $user->getVar("posts"));
...
class xforumUserHandler extends XoopsObjectHandler
{
    ...
    function &get($uid)
    {
	    ...
	    if ($xoopsModuleConfig['wol_enabled']) {
	        $userinfo["status"] = isset($this->status[$uid]) ?
	        	xforum_displayImage($xforumImage['online'], _MD_ONLINE) :
	        	xforum_displayImage($xforumImage['offline'],_MD_OFFLINE);
	    }
		return $userinfo;
    }
    ...
    {
	    $groups = xforum_getGroupsByUser(array_keys($users));
