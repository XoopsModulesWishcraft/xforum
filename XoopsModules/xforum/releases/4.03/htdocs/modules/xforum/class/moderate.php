...
class xforumModerateHandler extends ArtObjectHandler
{
    ...
    function verifyUser($uid=-1, $ip="", $xforum = 0){	    
	    ...
	    $ip = empty($ip)?xforum_getIP(true):$ip;
	    if(!empty($ip)){
		    $ip_segs = explode(".", $ip);
		    for($i=1; $i<=4; $i++){
	    ...
	    $xforum_criteria = empty($xforum)?"forum_id=0":"forum_id=0 OR forum_id=".intval($xforum);
	    $expire_criteria = "mod_end > ".time();
		$sql = sprintf("SELECT COUNT(*) AS count FROM %s WHERE (%s OR %s) AND (%s) AND (%s)", $this->db->prefix('xf_moderates'), $uid_criteria, $ip_criteria, $xforum_criteria, $expire_criteria);
    ...
    function forumList($uid=-1, $ip=""){
		...
		if(!empty($GLOBALS["xoopsModuleConfig"]['cache_enabled'])){
			$xforums[$uid][$ip] = xforum_getsession("sf".$uid."_".ip2long($ip), true);
		...
		if(!empty($GLOBALS["xoopsModuleConfig"]['cache_enabled'])){
			xforum_setsession("sf".$uid."_".ip2long($ip), $xforums[$uid][$ip]);
		}
