...
class Post extends ArtObject {
    ...
    function saveAttachment()
    {
        ...
        if (!$result = $GLOBALS["xoopsDB"]->queryF($sql)) {
            xforum_message("save attachment error: ". $sql);
            return false;
        }
        return true;
    }
        ...
        if (is_array($attachments) && count($attachments) > 0) {
            ...
            foreach($attachments as $key => $att) {
                ...
                if (in_array(strtolower($file_extension), $image_extensions) && $xoopsModuleConfig['media_allowed']) {
	                    ...
	                    $post_attachment .= '<br />' . xforum_attachmentImage($att['name_saved'], $asSource);
                		$isDisplayed = true;
                }else{
    ...
    function &getPostBody($imageAsSource = false)
    {
		...
		$post_text = xforum_displayTarea($this->vars['post_text']['value'], $this->getVar('dohtml'), $this->getVar('dosmiley'), $this->getVar('doxcode'), $this->getVar('doimage'), $this->getVar('dobr'));
        if (xforum_isAdmin($this->getVar('forum_id')) or $this->checkIdentity()) {
        ...
        $post['subject'] = xforum_htmlSpecialChars($this->vars['subject']['value']);
    ...
    {
        return xforum_checkTimelimit($this->getVar('post_time'), $action_tag);
    }
    ...
    function showPost($isadmin)
    {
		...
		$xforum_id = $this->getVar('forum_id');
		...
		if($GLOBALS["xoopsModuleConfig"]['enable_permcheck']){
	        ...
	        if ($topic_handler->getPermission($xforum_id, $topic_status, "edit")) {
	            $edit_ok = false;
	            if ($isadmin) {
	                $edit_ok = true;
	            ...
	            if ($edit_ok) {
	                ...
	                $thread_buttons['edit']['link'] = "edit.php?forum=" . $xforum_id . "&amp;topic_id=" . $topic_id . "&amp;viewmode=$viewmode&amp;order=$order";
	                $thread_buttons['edit']['name'] = _EDIT;
	            }
	        }
	
	        if ($topic_handler->getPermission($xforum_id, $topic_status, "delete")) {
	            $delete_ok = false;
	            if ($isadmin) {
	                $delete_ok = true;
	            ...
	            if ($delete_ok) {
	                ...
	                $thread_buttons['delete']['link'] = "delete.php?forum=" . $xforum_id . "&amp;topic_id=" . $topic_id . "&amp;viewmode=$viewmode&amp;order=$order";
	                $thread_buttons['delete']['name'] = _DELETE;
	            }
	        }
	        if ($topic_handler->getPermission($xforum_id, $topic_status, "reply")) {
	            $thread_buttons['reply']['image'] = xforum_displayImage($xforumImage['p_reply'], _MD_REPLY);
	            $thread_buttons['reply']['link'] = "reply.php?forum=" . $xforum_id . "&amp;topic_id=" . $topic_id . "&amp;viewmode=$viewmode&amp;order=$order&amp;start=$start";
	            $thread_buttons['reply']['name'] = _MD_REPLY;
	            /*
	            $thread_buttons['quote']['image'] = xforum_displayImage($xforumImage['p_quote'], _MD_QUOTE);
	            $thread_buttons['quote']['link'] = "reply.php?forum=" . $xforum_id . "&amp;topic_id=" . $topic_id . "&amp;viewmode=$viewmode&amp;order=$order&amp;start=$start&amp;quotedac=1";
        ...
        if (!$isadmin && $xoopsModuleConfig['reportmod_enabled']) {
            ...
            $thread_buttons['report']['link'] = "report.php?forum=" . $xforum_id . "&amp;topic_id=" . $topic_id . "&amp;viewmode=$viewmode&amp;order=$order";
        ...
        /*
        if ($isadmin) {
        	$thread_action['news']['image'] = xforum_displayImage($xforumImage['news'], _MD_POSTTONEWS);
        	$thread_action['news']['link'] = "posttonews.php?topic_id=" . $topic_id;
        	$thread_action['news']['name'] = _MD_POSTTONEWS;
        }

        $thread_action['pdf']['image'] = xforum_displayImage($xforumImage['pdf'], _MD_PDF);
        $thread_action['pdf']['link'] = "makepdf.php?type=post&amp;pageid=0&amp;scale=0.66";
        $thread_action['pdf']['name'] = _MD_PDF;

        $thread_action['print']['image'] = xforum_displayImage($xforumImage['printer'], _MD_PRINT);
        $thread_action['print']['link'] = "print.php?form=2&amp;forum=". $xforum_id."&amp;topic_id=" . $topic_id;
        ...
        */
        ...
        $post = array(
	                ...
	                'post_date' => xforum_formatTimestamp($this->getVar('post_time')),
...
class xforumPostHandler extends ArtObjectHandler
{
    ...
    function approve(&$post, $force = false)
    {
    	...
    	$xforum_obj =& $xforum_handler->get($post->getVar("forum_id"));
    	if($xforum_obj->getVar("forum_last_post_id") < $post->getVar("post_id")){
        	$xforum_obj->setVar("forum_last_post_id", $post->getVar("post_id"));
    	}
        $xforum_obj->setVar("forum_posts", $xforum_obj->getVar("forum_posts")+1);
        if ($post->isTopic()) {
	        $xforum_obj->setVar("forum_topics", $xforum_obj->getVar("forum_topics")+1);
        }
        $xforum_handler->insert($xforum_obj, true);
    ...
    function insertnewsubject($topic_id, $subject)
    {
        ...
        if (!$result) {
            xforum_message("update topic subject error:" . $sql);
            return false;
        }
        return true;
    }
    ...
    function &getPostsByLimit($criteria = null, $limit=1, $start=0, $join = null)
    {
        ...
        if (!$result) {
            xforum_message( "xforumPostHandler::getPostsByLimit error:" . $sql);
            return $ret;
        }
        while ($myrow = $this->db->fetchArray($result)) {
            $post =& $this->create(false);
    ...
    function cleanOrphan()
    {
        ...
        */
        ...
        if (!$result = $this->db->queryF($sql)) {
	        xforum_message("cleanOrphan:". $sql);
            return false;
        }
        return true;
    }
