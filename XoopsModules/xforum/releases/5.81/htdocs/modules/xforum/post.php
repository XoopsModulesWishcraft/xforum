...
if ( !empty($_POST['contents_submit']) ) {
    ...
    if ( !empty($isedit) && $post_id>0) {
        ...
        $isreply = 0;
        $isnew = 1;
        if ( !is_object($xoopsUser) || ( !empty($_POST['noname']) && !empty($GLOBALS['xforumModuleConfig']['allow_user_anonymous']) ) ) {
            $uid = 0;
        }
        else {
	...
	if ($GLOBALS['xoopsModuleConfig']['tag']&&file_exists(XOOPS_ROOT_PATH . '/modules/tag/class/tag.php')) {
		$tag_handler = xoops_getmodulehandler('tag', 'tag');
		$tag_handler->updateByItem($_POST['tags'], $postid, $GLOBALS['xoopsModule']->getVar("dirname"));
	}
    ...
    if (!empty($xoopsUser) && !empty($GLOBALS['xforumModuleConfig']['notification_enabled'])) {
	    $notification_handler =& xoops_gethandler('notification');
