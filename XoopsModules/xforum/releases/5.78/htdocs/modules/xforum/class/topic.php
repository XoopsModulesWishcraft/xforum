...
class Topic extends XoopsObject 
{
    ...
    {	
		if ($GLOBALS['xoopsModuleConfig']['htaccess']) {
    		return $this->domain() . '/' . $GLOBALS['xoopsModuleConfig']['baseurl'] . $this->getSUBPath() . '/archive,' . $this->getVar('forum_id') . ',' . $this->getVar('topic_id') . $GLOBALS['xoopsModuleConfig']['endofurl'];
    	} else {
    ...
    function getURL($post_id=0) {
    	...
    	if ($post_id>0||($start>0&&!isset($_GET['sortname']))) {
    		if ($GLOBALS['xoopsModuleConfig']['htaccess']) {
	    		return $this->domain() . '/' . $GLOBALS['xoopsModuleConfig']['baseurl'] . $this->getSUBPath() . '/' . xoops_sef($this->getVar('topic_title')) . '/' . $this->getVar('forum_id') . ',' . $this->getVar('topic_id')  . ",$post_id,$start,$move,$type,$order" . $GLOBALS['xoopsModuleConfig']['endofurl'] . '#forumpost' . $post_id;
	    	} else {
    		...
    		if (strlen($viewmode)>0) {
    			if ($GLOBALS['xoopsModuleConfig']['htaccess']) {
			    	return $this->domain() . '/' . $GLOBALS['xoopsModuleConfig']['baseurl'] . $this->getSUBPath() . '/' . xoops_sef($this->getVar('topic_title')) . '/' . $this->getVar('forum_id') . ',' . $this->getVar('topic_id')  . ",$viewmode" . $GLOBALS['xoopsModuleConfig']['endofurl'];
			    } else {
	    		...
	    		if ($GLOBALS['xoopsModuleConfig']['htaccess']) {
			    	return $this->domain() . '/' . $GLOBALS['xoopsModuleConfig']['baseurl'] . $this->getSUBPath() . '/' . xoops_sef($this->getVar('topic_title')) . '/' . $this->getVar('forum_id') . ',' . $this->getVar('topic_id')  . "" . $GLOBALS['xoopsModuleConfig']['endofurl'];
			    } else {
    ...
    function getURL_JUMPBOX($start=0, $post_id=0) {
		...
		if (empty($post_id))
    		$post_id = $this->getVar('topic_last_post_id');
