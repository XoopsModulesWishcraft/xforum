...
function forum_admin_getPathStatus($path)
{
	...
	if(@is_writable($path)){
		...
		$path_status = _AM_XFORUM_NOTAVAILABLE." <a href=admin_dashboard.php?op=createdir&amp;path=$path>"._AM_XFORUM_CREATETHEDIR.'</a>';
	}else{
		$path_status = _AM_XFORUM_NOTWRITABLE." <a href=admin_dashboard.php?op=setperm&amp;path=$path>"._AM_XFORUM_SETMPERM.'</a>';
	}
	return $path_status;
}
