...
if (!$topic_handler->getPermission($GLOBALS['viewtopic_forum'], $GLOBALS['xforumtopic']->getVar('topic_status'), "view")){
    redirect_header("viewforum.php?forum=".$GLOBALS['viewtopic_forum']->getVar('forum_id'),2,_MD_NORIGHTTOVIEW);
    exit();
}
...
if(!$perm){
    redirect_header("viewtopic.php?topic_id=".$topic_id."&forum=".$GLOBALS['viewtopic_forum']->getVar('forum_id'),2,_MD_NORIGHTTOACCESS);
}
...
if ( $op == "save" ) {
	...
	redirect_header("viewtopic.php?topic_id=$topic_id&forum=".$GLOBALS['viewtopic_forum']->getVar('forum_id'),1,_MD_POLL_DBUPDATED);
	//exit();
}
