...
case "list":
	...
	$forums = $forum_handler->getObjects($criteria, true);
	unset($criteria);
...
case "edit":
	...
	if ( !$obj->getVar('field_config') && !$obj->getVar('field_show') && !$obj->getVar('field_edit')  ) { //If no configs exist
		redirect_header('field.php', 2, _AM_XFORUM_FIELDNOTCONFIGURABLE);
	}
	include_once('../include/functions.php');	
	$form = xforum_getFieldForm($obj);
	$form->display();
...
case "reorder":
	...
	if ( isset($_POST['field_ids']) && count($_POST['field_ids']) > 0 ) {
		...
		if ( count($ids) > 0 ) {
			...
			if ( count($errors) == 0 ) {
				//no errors
				redirect_header('field.php', 2, sprintf(_AM_XFORUM_SAVEDSUCCESS, _AM_XFORUM_FIELDS) );
			} else {
...
case "save":
	...
	if ( isset($_REQUEST['id'])  ) {
		$obj =& $profilefield_handler->get($_REQUEST['id']);
		if ( !$obj->getVar('field_config') && !$obj->getVar('field_show') && !$obj->getVar('field_edit')  ) { //If no configs exist
			redirect_header('admin.php', 2, _AM_XFORUM_FIELDNOTCONFIGURABLE);
		}
	} else {
		$obj =& $profilefield_handler->create();
	...
	if ( $profilefield_handler->insert($obj)  ) {
		...
		if ( $obj->getVar('field_show')  ) {
			$perm_arr[] = 'objects_show';
			$perm_arr[] = 'objects_visible';
		}
		if ( $obj->getVar('field_edit')  ) {
			$perm_arr[] = 'objects_edit';
		}
		if ( $obj->getVar('field_edit') || $obj->getVar('field_show')  ) {
			$perm_arr[] = 'objects_search';
		}
		if ( count($perm_arr) > 0 ) {
				...
				if ( isset($_REQUEST[$perm]) && is_array($_REQUEST[$perm])  ) {
					...
					if ( count($perms) > 0 ) {
						foreach (array_keys($perms) as $i ) {
							$groups[$perms[$i]->getVar('gperm_grouoid')] =& $perms[$i];
						}
					} else {
						$groups = array();
					}
					foreach ($_REQUEST[$perm] as $grouoid ) {
						...
						if ( !isset($groups[$grouoid])  ) {
							...
							$perm_obj->setVar('gperm_grouoid', $grouoid);
					...
					if ( count($removed_groups) > 0 ) {
						$criteria->add(new Criteria('gperm_grouoid', "(".implode(',', $removed_groups).")", "IN") );
...
case "delete":
	...
	if ( isset($_REQUEST['ok']) && $_REQUEST['ok'] == 1 ) {
		...
		if ( $profilefield_handler->delete($obj)  ) {
			redirect_header('field.php', 3, sprintf(_AM_XFORUM_DELETEDSUCCESS, _AM_XFORUM_FIELD) );
		} else {
			echo $obj->getHtmlErrors();
		}
	} else {
