...
if ( !$topic_id && !$post_id ) {
    ...
    $GLOBALS['xoops']->redirect($redirect, 2, _MD_ERRORTOPIC);
}
...
if ( !is_object($GLOBALS['xforumtopic']) || !$topic_id = $GLOBALS['xforumtopic']->getVar('topic_id') ) {
    $GLOBALS['xoops']->redirect(XOOPS_URL.'/modules/xforum/viewforum.php?forum='.$forum_id, 2, _MD_ERRORTOPIC);
}
...
if(!$isadmin && $GLOBALS['xforumtopic']->getVar('approved')<0 ){
    $GLOBALS['xoops']->redirect($GLOBALS['viewtopic_forum']->getURL(),2,_MD_NORIGHTTOVIEW);
    exit();
}
...
$perm = $GLOBALS['xoops']->getModuleHandler('permission', 'xforum');
...
if ($viewmode == "thread") {
    ...
    if(!empty($GLOBALS['xforumModuleConfig']["posts_for_thread"]) && $GLOBALS['total_posts'] > $GLOBALS['xforumModuleConfig']["posts_for_thread"]) {
	    $GLOBALS['xoops']->redirect("viewtopic.php?topic_id=$topic_id&amp;viewmode=flat", 2, _MD_EXCEEDTHREADVIEW);
	    exit();
    }
...
if(!empty($GLOBALS['xforumModuleConfig']['rss_enable'])){
	$GLOBALS['xoops']->theme->addLink('alternate', $GLOBALS['viewtopic_forum']->getRSSURL(), array('type'=>"application/rss+xml", 'title' => $GLOBALS['xforumModule']->getVar('name')));
}
...
if($GLOBALS['viewtopic_forum']->getVar('parent_forum') > 0){
    ...
    $GLOBALS['xoops']->tpl->assign(array('parent_forum' => $GLOBALS['viewtopic_forum']->getVar('parent_forum'), 'parent_name' => $GLOBALS['myts']->htmlSpecialChars($row['forum_name'])));
}
...
$GLOBALS['xoops']->tpl->assign('category', array("id" => $GLOBALS['viewtopic_forum']->getVar("cat_id"), "title" => $category_obj->getVar('cat_title')));

$GLOBALS['xoops']->tpl->assign('folder_topic', forum_displayImage($GLOBALS['xforumImage']['folder_topic']));

$GLOBALS['xoops']->tpl->assign('topic_id', $topic_id);
$GLOBALS['xoops']->tpl->assign('forum_id', $forum_id);

if ($order == 'DESC') {
	$order_current = 'DESC';
    $GLOBALS['xoops']->tpl->assign(array('order_current' => 'DESC'));
} else {
	$order_current = 'ASC';
    $GLOBALS['xoops']->tpl->assign(array('order_current' => 'ASC'));
}
...
if ($topic_handler->getPermission($GLOBALS['viewtopic_forum'], $GLOBALS['xforumtopic']->getVar('topic_status'), "post")){
    $GLOBALS['xoops']->tpl->assign('forum_post_or_register', "<a href=\"".XOOPS_URL."/modules/xforum/newtopic.php?forum=".$forum_id."\">".$t_new."</a>");
} elseif ( !empty($GLOBALS['xforumModuleConfig']["show_reg"]) ) {
    if($GLOBALS['xforumtopic']->getVar('topic_status')){
    	$GLOBALS['xoops']->tpl->assign('forum_post_or_register', _MD_TOPICLOCKED);
    }elseif ( !is_object($GLOBALS['xoopsUser'])) {
    	$GLOBALS['xoops']->tpl->assign('forum_post_or_register', '<a href="'.XOOPS_URL.'/user.php?xoops_redirect='.htmlspecialchars($xoopsRequestUri).'">'._MD_REGTOPOST.'</a>');
	}
} else {
...
if ($topic_handler->getPermission($GLOBALS['viewtopic_forum'], $GLOBALS['xforumtopic']->getVar('topic_status'), "reply")){
    $GLOBALS['xoops']->tpl->assign('forum_reply', "<a href=\"".XOOPS_URL."/modules/xforum/reply.php?forum=".$forum_id."&amp;topic_id=".$topic_id."\">".$t_reply."</a>");
}
...
if ($viewmode == "thread") {
	if(!empty($post_id)){
		...
		if(!$isadmin && $currentPost->getVar('approved')<0 ){
		    $GLOBALS['xoops']->redirect("viewtopic.php?topic_id=".$topic_id, 2, _MD_NORIGHTTOVIEW);
		    exit();
		}
    ...
    $GLOBALS['xoops']->tpl->append('topic_posts', $currentPost->showPost($isadmin));
    ...
    if ( count($postArray) > 0 ) {
        foreach ($postArray as $treeItem) {
            ...
            $GLOBALS['xoops']->tpl->append("topic_trees", array("post_id" => $treeItem['post_id'], "post_time" => $treeItem['post_time'], "post_image" => $treeItem['icon'], "post_title" => $treeItem['subject'], "post_prefix" => $treeItem['prefix'], "poster" => $treeItem['poster']));
        }
        unset($postArray);
    }
}
    ...
    foreach ($postsArray as $eachpost) {
    	$GLOBALS['xoops']->tpl->append('topic_posts', $eachpost->showPost($isadmin));
    }
    ...
    if ( $GLOBALS['total_posts'] > $GLOBALS['xforumModuleConfig']['posts_per_page'] ) {
        ...
        $GLOBALS['xoops']->tpl->assign('forum_page_nav', $nav->renderNav(4));
    } else {
        $GLOBALS['xoops']->tpl->assign('forum_page_nav', '');
    }
}
unset($postsArray);

$GLOBALS['xoops']->tpl->assign('topic_print_link', "".XOOPS_URL."/modules/xforum/print.php?form=1&amp;topic_id=$topic_id&amp;forum=".$forum_id."&amp;order=$order&amp;start=$start");
...
$GLOBALS['xoops']->tpl->assign_by_ref('admin_actions', $admin_actions);
...
if ( $GLOBALS['xforumtopic']->getVar('topic_haspoll') 
	...
	$GLOBALS['xoops']->tpl->assign('topic_poll', 1);
	...
	if ( XoopsPollLog::hasVoted($GLOBALS['xforumtopic']->getVar('poll_id'), $_SERVER['REMOTE_ADDR'], $uid) ) {
		$renderer->assignResults($GLOBALS['xoops']->tpl);
		//pollresults($GLOBALS['xforumtopic']->getVar('poll_id'));
		$GLOBALS['xoops']->tpl->assign('topic_pollresult', 1);
		setcookie("xf_polls[".$GLOBALS['xforumtopic']->getVar("poll_id")."]", 1);
	} else {
	    $renderer->assignForm($GLOBALS['xoops']->tpl);
	    $GLOBALS['xoops']->tpl->assign('lang_vote' , _PL_VOTE);
	    $GLOBALS['xoops']->tpl->assign('lang_results' , _PL_RESULTS);
...
if ($topic_handler->getPermission($GLOBALS['viewtopic_forum'], $GLOBALS['xforumtopic']->getVar('topic_status'), "addpoll")
	...
	if(!$GLOBALS['xforumtopic']->getVar('topic_haspoll')){	
		if( is_object($GLOBALS['xoopsUser']) && $GLOBALS['xoopsUser']->getVar("uid")==$GLOBALS['xforumtopic']->getVar("topic_poster") ){
			...
			$GLOBALS['xoops']->tpl->assign('forum_addpoll', "<a href=\"".XOOPS_URL."/modules/xforum/polls.php?op=add&amp;forum=".$forum_id."&amp;topic_id=".$topic_id."\">".$t_poll."</a>&nbsp;");
		}
	}elseif($isadmin
	    ...
	    $GLOBALS['xoops']->tpl->assign_by_ref('adminpoll_actions', $adminpoll_actions);	
...
$GLOBALS['xoops']->tpl->assign('threaded',forum_displayImage($GLOBALS['xforumImage']['threaded']));
$GLOBALS['xoops']->tpl->assign('flat',forum_displayImage($GLOBALS['xforumImage']['flat']));
$GLOBALS['xoops']->tpl->assign('left',forum_displayImage($GLOBALS['xforumImage']['left']));
$GLOBALS['xoops']->tpl->assign('right',forum_displayImage($GLOBALS['xforumImage']['right']));
...
if(!empty($GLOBALS['xforumModuleConfig']['rating_enabled'])){
	$GLOBALS['xoops']->tpl->assign('votes',$GLOBALS['xforumtopic']->getVar('votes'));
	...
	$GLOBALS['xoops']->tpl->assign('rating_img',$rating_img);
	$GLOBALS['xoops']->tpl->assign('rate1',forum_displayImage($GLOBALS['xforumImage']['rate1'],_MD_RATE1));
	$GLOBALS['xoops']->tpl->assign('rate2',forum_displayImage($GLOBALS['xforumImage']['rate2'],_MD_RATE2));
	$GLOBALS['xoops']->tpl->assign('rate3',forum_displayImage($GLOBALS['xforumImage']['rate3'],_MD_RATE3));
	$GLOBALS['xoops']->tpl->assign('rate4',forum_displayImage($GLOBALS['xforumImage']['rate4'],_MD_RATE4));
	$GLOBALS['xoops']->tpl->assign('rate5',forum_displayImage($GLOBALS['xforumImage']['rate5'],_MD_RATE5));
}
...
$GLOBALS['xoops']->tpl->assign('topictype', $current_type);
...
if( !empty($GLOBALS['xforumModuleConfig']['quickreply_enabled'])
	...
	if(!$editor_handler = $GLOBALS['xoops']->getHandler("editor", true)){
    ...
    $GLOBALS['xoops']->tpl->assign('quickreply', array( 'show' => 1, 'display'=>$display, 'icon'=>forum_displayImage($GLOBALS['xforumImage']['t_qr']), 'form' => $forum_form->render()));
	unset($forum_form);
}else{
	$GLOBALS['xoops']->tpl->assign('quickreply', array( 'show' => 0));
}
