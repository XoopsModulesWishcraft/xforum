...
if ( empty($forum) ) {
    $GLOBALS['xoops']->redirect(XOOPS_URL."/index.php", 2, _MD_ERRORFORUM);
    exit();
}
...
if (!$xoopsCaptcha->verify()) {
	$GLOBALS['xoops']->redirect(XOOPS_URL.'/modules/xforum/', 10, $xoopsCaptcha->getMessage());
	exit(0);
}
...
if ( !empty($_POST['contents_submit']) ) {
	...
	if(empty($message)){
	    $GLOBALS['xoops']->redirect("javascript:history.go(-1);", 1);
	    exit();
	}
    if ( !empty($isedit) && $post_id>0) {
		...
		if ( $topic_handler->getPermission($forum_obj, $GLOBALS['topic_status'], 'edit')
		    ...
		    $GLOBALS['xoops']->redirect(XOOPS_URL."/modules/xforum/viewtopic.php?forum=$forum_id&amp;topic_id=$topic_id&amp;post_id=$post_id&amp;order=$order&amp;viewmode=$viewmode&amp;pid=$pid",2,_MD_NORIGHTTOEDIT);
		    exit();
		}
		...
		if($topic_id){
			...
			if (!$topic_handler->getPermission($forum_obj, $GLOBALS['topic_status'], 'reply')) {
			    $GLOBALS['xoops']->redirect(XOOPS_URL."/modules/xforum/viewtopic.php?forum=$forum_id&amp;topic_id=$topic_id&amp;post_id=$post_id&amp;order=$order&amp;viewmode=$viewmode&amp;pid=$pid",2,_MD_NORIGHTTOREPLY);
			    exit();
			}
		}else{
			$GLOBALS['topic_status'] = 0;
			if (!$topic_handler->getPermission($forum_obj, $GLOBALS['topic_status'], 'post')) {
			    $GLOBALS['xoops']->redirect(XOOPS_URL."/modules/xforum/viewtopic.php?forum=$forum_id",2,_MD_NORIGHTTOPOST);
			    exit();
			}
		}
	...
	$objects_handler = $GLOBALS['xoops']->getModuleHandler('post');
	$profile_handler = $GLOBALS['xoops']->getModuleHandler('extras');
    ...
    if ($postid = $post_handler->insert($forumpost, true, __FILE__)) {
		...
		if (!$profile_handler->insert($profile)) {
			...
			$GLOBALS['xoops']->error('Could not insert forum post extras');
			$GLOBALS['xoops']->error($profile->getErrors());
			include_once(XOOPS_ROOT_PATH.'/footer.php');
			exit();
		}
	} else {
        include_once(XOOPS_ROOT_PATH.'/header.php');
        $GLOBALS['xoops']->error('Could not insert forum post');
	    $GLOBALS['xoops']->error($forumpost->getErrors());
        include_once(XOOPS_ROOT_PATH.'/footer.php');
        exit();
    }

	if ($GLOBALS['xforumModuleConfig']['tag']&&file_exists(XOOPS_ROOT_PATH . '/modules/tag/class/tag.php')) {
		$tag_handler = $GLOBALS['xoops']->getModuleHandler('tag', 'tag');
    ...
    if($approved && !empty($GLOBALS['xforumModuleConfig']['notification_enabled']) && !empty($isnew)){
	    ...
	    $notification_handler = $GLOBALS['xoops']->getHandler('notification');
        if (empty($isreply)) {
	...
	if ( $op == "add" ) {
		$GLOBALS['xoops']->redirect(XOOPS_URL."/modules/xforum/polls.php?op=add&amp;forum=".$forumpost->getVar('forum_id')."&amp;topic_id=".$forumpost->getVar('topic_id')."",1,_MD_ADDPOLL);
		exit();
    }else{
	    $GLOBALS['xoops']->redirect($redirect,2,$message);
        exit();
    }
}
