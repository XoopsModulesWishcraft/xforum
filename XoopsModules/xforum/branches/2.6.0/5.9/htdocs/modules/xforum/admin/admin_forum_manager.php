...
switch ($op) {
    case 'moveforum':
        ...
        if(!empty($_POST['dest'])) {
            ...
            if (!$bMoved) {
                $GLOBALS['xoops']->redirect('./admin_forum_manager.php?op=manage', 2, _AM_XFORUM_MSG_ERR_FORUM_MOVED);
            } else {
                $GLOBALS['xoops']->redirect('./admin_forum_manager.php?op=manage', 2, _AM_XFORUM_MSG_FORUM_MOVED);
            }
            exit();
        } else {
	        $indexAdmin = new ModuleAdmin();
    ...
    case 'mergeforum':
        ...
        if (!empty($_POST['dest_forum'])) {
            ...
            if ($post_count || $topic_count) {
                $GLOBALS['xoops']->redirect('./admin_forum_manager.php?op=manage', 2, _AM_XFORUM_MSG_ERR_FORUM_MERGED);
            } else {
                $GLOBALS['xoops']->redirect('./admin_forum_manager.php?op=manage', 2, _AM_XFORUM_MSG_FORUM_MERGED);
            }
            exit();
        } else {
            $indexAdmin = new ModuleAdmin();
    ...
    case 'sync':
        ...
        if (isset($_POST['submit'])) {
			...
			*/
            $GLOBALS['xoops']->redirect("./index.php", 1, _AM_XFORUM_SYNCHING);
            exit();
        } else {
    ...
    case "save":
        ...
        if ($forum_handler->insert($ff)) {
	        if(!empty($_POST["perm_template"])){
			    $groupperm_handler = $GLOBALS['xoops']->getModuleHandler('permission', 'xforum');
            ...
            $GLOBALS['xoops']->redirect("admin_forum_manager.php?op=mod&amp;forum=" . $ff->getVar('forum_id') . "", 2, $message);
            exit();
        } else {
            $GLOBALS['xoops']->redirect("admin_forum_manager.php?op=mod&amp;forum=" . $ff->getVar('forum_id') . "", 2, _AM_XFORUM_FORUM_ERROR);
            exit();
        }
    ...
    case "del":
        ...
        if (isset($_POST['confirm']) != 1) {
            ...
            $GLOBALS['xoops']->redirect("admin_forum_manager.php?op=manage", 1, _AM_XFORUM_FORUMREMOVED);
            exit();
        }
        break;
...
echo chronolabs_inline(false); $GLOBALS['xoops']->footer();
