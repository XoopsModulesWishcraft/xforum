...
switch($action){
	case "template":
        ...
        $glist = $member_handler->getGroupList();
        $elements = array();
        $xforumperm_handler = $GLOBALS['xoops']->getModuleHandler('permission', 'xforum');
	...
	case "template_save":
        ...
        if($res){
	    	$GLOBALS['xoops']->redirect("admin_permissions.php?action=template", 2, _AM_XFORUM_PERM_TEMPLATE_CREATED);
        }else{
	    	$GLOBALS['xoops']->redirect("admin_permissions.php?action=template", 2, _AM_XFORUM_PERM_TEMPLATE_ERROR);
        }
		break;
	...
	case "apply":
		...
		if($perm_template===null){
	    	$GLOBALS['xoops']->redirect("admin_permissions.php?action=template", 2, _AM_XFORUM_PERM_TEMPLATE);
		}
	...
	case "apply_save":
	    ...
	    $GLOBALS['xoops']->redirect("admin_permissions.php", 2, _AM_XFORUM_PERM_TEMPLATE_APPLIED);
		break;
