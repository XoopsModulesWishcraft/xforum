...
class Forum extends XoopsObject {
    ...
    function getSUBPath()
    {
		...
		if (!isset($subpath[$this->getVar('forum_id')])) {
	    	...
	    	$categories_handler = $GLOBALS['xoops']->getModuleHandler('category', 'xforum');
    ...
    function getURL()
    {
		...
		if ($GLOBALS['xforumModuleConfig']['htaccess']) {
    		return urldecode($this->getVar('domain')) . '/' . $GLOBALS['xforumModuleConfig']['baseurl'] . $this->getSUBPath() . '/' . $this->getVar('forum_id') . ",$since,$start,$sortorder,$sortname,$mode,$type" . $GLOBALS['xforumModuleConfig']['endofurl'];
    	} else {
    ...
    {	   	
		if ($GLOBALS['xforumModuleConfig']['htaccess']) {
    		return urldecode($this->getVar('domain')) . '/' . $GLOBALS['xforumModuleConfig']['baseurl'] . $this->getSUBPath() . '/reply,' . $xpost->getVar('forum_id').','.$xpost->getVar('topic_id').','.$xpost->getVar('post_id') . $GLOBALS['xforumModuleConfig']['endofurl'];
    	} else {
    ...
    {	
		...
		if ($GLOBALS['xforumModuleConfig']['htaccess']) {
    		return urldecode($this->getVar('domain')) . '/' . $GLOBALS['xforumModuleConfig']['baseurl'] . $this->getSUBPath() . '/poll,' . $op . ',' . $topic_id . ','  . $poll_id . $GLOBALS['xforumModuleConfig']['endofurl'];
    	} else {
    ...
    {	
    	if (isset($_GET['op'])) {
	    	if ($GLOBALS['xforumModuleConfig']['htaccess']) {
	    		return urldecode($this->getVar('domain')) . '/' . $GLOBALS['xforumModuleConfig']['baseurl'] . $this->getSUBPath() . '/newtopic,' . $_GET['op'] . ',' . $this->getVar('forum_id') . $GLOBALS['xforumModuleConfig']['endofurl'];
	    	} else {
	    		return urldecode($this->getVar('domain')) . '/modules/xforum/newtopic.php?forum='.$this->getVar('forum_id').'&op='.$_GET['op'];
	    	}	
    	} else {
			if ($GLOBALS['xforumModuleConfig']['htaccess']) {
	    		return urldecode($this->getVar('domain')) . '/' . $GLOBALS['xforumModuleConfig']['baseurl'] . $this->getSUBPath() . '/newtopic,' . $this->getVar('forum_id') . $GLOBALS['xforumModuleConfig']['endofurl'];
	    	} else {
    ...
    {	
		if ($GLOBALS['xforumModuleConfig']['htaccess']) {
    		return urldecode($this->getVar('domain')) . '/' . $GLOBALS['xforumModuleConfig']['baseurl'] . $this->getSUBPath() . '/edit,' . $this->getVar('forum_id') . ',' . $post_id . $GLOBALS['xforumModuleConfig']['endofurl'];
    	} else {
    ...
    {	
		if ($GLOBALS['xforumModuleConfig']['htaccess']) {
    		return urldecode($this->getVar('domain')) . '/' . $GLOBALS['xforumModuleConfig']['baseurl'] . $this->getSUBPath() . '/archive,' . $this->getVar('forum_id') . $GLOBALS['xforumModuleConfig']['endofurl'];
    	} else {
    ...
    {
    	if ($GLOBALS['xforumModuleConfig']['htaccess']) {
    		return urldecode($this->getVar('domain')) . '/' . $GLOBALS['xforumModuleConfig']['baseurl'] . '/rss,0,' . $this->getVar('forum_id') . $GLOBALS['xforumModuleConfig']['endofurl_rss'];
    	} else {
    ...
    {
    	if ($GLOBALS['xforumModuleConfig']['htaccess']) {
    		return urldecode($this->getVar('domain')) . '/' . $GLOBALS['xforumModuleConfig']['baseurl'] . $this->getSUBPath() . '/' . $this->getVar('forum_id') . $GLOBALS['xforumModuleConfig']['endofurl_pdf'];
    	} else {
...
class xforumForumHandler extends XoopsPersistableObjectHandler
{
    ...
    function insert($xforum)
    {
        if(!parent::insert($xforum, true)){
	        $GLOBALS['xoops']->error($xforum->getErrors());
	        return false;
        }
    ...
    function getAllTopics($xforum, $startdate, $start, $sortname, $sortorder, $type = '', $excerpt = 0)
    {
        ...
        if (!$result = $this->db->query($sql, $GLOBALS['xforumModuleConfig']['topics_per_page'], $GLOBALS['start'])) {
            $GLOBALS['xoops']->redirect('index.php', 2, _MD_ERROROCCURED . '<br />' . $sql);
            exit();
        }
        ...
        $topics_handler = $GLOBALS['xoops']->getModuleHandler('topic', 'xforum');
    ...
    {
		$perm_handler = $GLOBALS['xoops']->getModuleHandler('permission', 'xforum');
    ...
    function cleanOrphan()
    {
        ...
        */
        ...
        if (!$result = $this->db->queryF($sql)):
	        $GLOBALS['xoops']->error("cleanOrphan error:". $sql);
        endif;
        else:
    ...
    function synchronization($object = null)
    {
	    ...
	    if(empty($object)) {
			...
			if(!$this->db->queryF($sql)){
				$GLOBALS['xoops']->error("update error: ".$sql);
			}
	        $sql = "UPDATE ".$this->table.
