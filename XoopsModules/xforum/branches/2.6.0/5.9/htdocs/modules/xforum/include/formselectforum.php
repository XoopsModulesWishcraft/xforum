...
XoopsLoad::load('XoopsFormElement');

class XoopsFormSelectForum extends XoopsFormElement
{
    /**
     * Options
     *
     ...
     */
    function XoopsFormSelectForum($caption, $name, $value = null, $size = 1, $multiple = false, $ownid = 0, $nonone = false)
    {
		...
		foreach ($this->GetCategories($ownid, $searchable) as $id => $object)
			$this->addOption($id, $object);
    }

	function GetForums($ownid, $searchable){
		$category_handler = $GLOBALS['xoops']->getModuleHandler('forum', 'xforum');
		$categories = $category_handler->getObjects($criteria, true);
		$langs_array = $this->TreeMenu(array(), $categories, -1, $ownid, $searchable);
		return $langs_array;
	}

	function TreeMenu($langs_array, $categories, $level, $ownid=null, $searchable=false) {

		$level++;
		$category_handler = $GLOBALS['xoops']->getModuleHandler('forum', 'xforum');
		foreach($categories as $catid => $category) {
			if ($catid!=$ownid) {
				$langs_array[$catid] = str_repeat('--', $level) . $category->getVar('forum_name');
				$criteria = new CriteriaCompo(new Criteria('parent_forum', $catid));
