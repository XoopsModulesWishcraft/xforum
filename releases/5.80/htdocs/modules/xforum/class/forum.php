...
class Forum extends XoopsObject {
    ...
    function getURL()
    {
		...
		if ($GLOBALS['xoopsModuleConfig']['htaccess']) {
    		return urldecode($this->getVar('domain')) . '/' . $GLOBALS['xoopsModuleConfig']['baseurl'] . $this->getSUBPath() . '/' . $this->getVar('forum_id') . ",$since,$start,$sortorder,$sortname,$mode,$type" . $GLOBALS['xoopsModuleConfig']['endofurl'];
    	} else {
    		return urldecode($this->getVar('domain')) . '/modules/xforum/viewforum.php?forum='.$this->getVar('forum_id') . "&amp;type=$type&amp;mode=$mode&amp;since=$since&amp;start=$start&amp;sortorder=$sortorder&amp;sortname=$sortname";
    	}
    }
    ...
    {	   	
		if ($GLOBALS['xoopsModuleConfig']['htaccess']) {
    		return urldecode($this->getVar('domain')) . '/' . $GLOBALS['xoopsModuleConfig']['baseurl'] . $this->getSUBPath() . '/reply,' . $xpost->getVar('forum_id').','.$xpost->getVar('topic_id').','.$xpost->getVar('post_id') . $GLOBALS['xoopsModuleConfig']['endofurl'];
    	} else {
    		return urldecode($this->getVar('domain')) . '/modules/xforum/reply.php?forum='.$xpost->getVar('forum_id').'&amp;topic_id='.$xpost->getVar('topic_id').'&amp;post_id='.$xpost->getVar('post_id');
    	}
    }
    ...
    {	
		...
		if ($GLOBALS['xoopsModuleConfig']['htaccess']) {
    		return urldecode($this->getVar('domain')) . '/' . $GLOBALS['xoopsModuleConfig']['baseurl'] . $this->getSUBPath() . '/poll,' . $op . ',' . $topic_id . ','  . $poll_id . $GLOBALS['xoopsModuleConfig']['endofurl'];
    	} else {
    		return urldecode($this->getVar('domain')) . '/modules/xforum/poll.php?op='.$op.'&amp;topic_id='.$topic_id.'&amp;poll_id='.$poll_id;
    	}
    }
    ...
    {	
    	if (isset($_GET['op'])) {
	    	if ($GLOBALS['xoopsModuleConfig']['htaccess']) {
	    		return urldecode($this->getVar('domain')) . '/' . $GLOBALS['xoopsModuleConfig']['baseurl'] . $this->getSUBPath() . '/newtopic,' . $_GET['op'] . ',' . $this->getVar('forum_id') . $GLOBALS['xoopsModuleConfig']['endofurl'];
	    	} else {
	    		return urldecode($this->getVar('domain')) . '/modules/xforum/newtopic.php?forum='.$this->getVar('forum_id').'&op='.$_GET['op'];
	    	}	
    	} else {
			if ($GLOBALS['xoopsModuleConfig']['htaccess']) {
	    		return urldecode($this->getVar('domain')) . '/' . $GLOBALS['xoopsModuleConfig']['baseurl'] . $this->getSUBPath() . '/newtopic,' . $this->getVar('forum_id') . $GLOBALS['xoopsModuleConfig']['endofurl'];
	    	} else {
	    		return urldecode($this->getVar('domain')) . '/modules/xforum/newtopic.php?forum='.$this->getVar('forum_id').'';
	    	}
    	}
    }
    ...
    {	
		if ($GLOBALS['xoopsModuleConfig']['htaccess']) {
    		return urldecode($this->getVar('domain')) . '/' . $GLOBALS['xoopsModuleConfig']['baseurl'] . $this->getSUBPath() . '/edit,' . $this->getVar('forum_id') . ',' . $post_id . $GLOBALS['xoopsModuleConfig']['endofurl'];
    	} else {
    		return urldecode($this->getVar('domain')) . '/modules/xforum/edit.php?forum='.$this->getVar('forum_id').'&amp;post_id='.$post_id;
    	}
    }
    ...
    {	
		if ($GLOBALS['xoopsModuleConfig']['htaccess']) {
    		return urldecode($this->getVar('domain')) . '/' . $GLOBALS['xoopsModuleConfig']['baseurl'] . $this->getSUBPath() . '/archive,' . $this->getVar('forum_id') . $GLOBALS['xoopsModuleConfig']['endofurl'];
    	} else {
    		return urldecode($this->getVar('domain')) . '/modules/xforum/archive.php?forum='.$this->getVar('forum_id');
    	}
    }
    ...
    {
    	if ($GLOBALS['xoopsModuleConfig']['htaccess']) {
    		return urldecode($this->getVar('domain')) . '/' . $GLOBALS['xoopsModuleConfig']['baseurl'] . '/rss,0,' . $this->getVar('forum_id') . $GLOBALS['xoopsModuleConfig']['endofurl_rss'];
    	} else {
    		return urldecode($this->getVar('domain')) . '/modules/xforum/rss.php?f='.$this->getVar('forum_id');
    	}
    }
    ...
    {
    	if ($GLOBALS['xoopsModuleConfig']['htaccess']) {
    		return urldecode($this->getVar('domain')) . '/' . $GLOBALS['xoopsModuleConfig']['baseurl'] . $this->getSUBPath() . '/' . $this->getVar('forum_id') . $GLOBALS['xoopsModuleConfig']['endofurl_pdf'];
    	} else {
    		return urldecode($this->getVar('domain')) . '/modules/xforum/makepdf.php?forum_id='.$this->getVar('forum_id');
    	}
    }
...
class xforumForumHandler extends XoopsPersistableObjectHandler
{
    ...
    function &getForums($cat = 0, $permission = "", $tags = null, $admin = false)
    {
        ...
        if ($GLOBALS['xoopsModuleConfig']['multisite']&&$admin==false) { 
        	$criteria->add(new Criteria('domains', '%'.urlencode(XOOPS_URL).'%', 'LIKE'));
			$criteria->add(new Criteria('domains', '%"all"%', 'LIKE'), 'OR');
        }
        if ($GLOBALS['xoopsModuleConfig']['multilingual']&&$admin==false) 
    ...
    function getAllTopics($xforum, $startdate, $start, $sortname, $sortorder, $type = '', $excerpt = 0)
    {
        ...
        if ($GLOBALS['xoopsModuleConfig']['multilingual']) {
            ...
            if ($totalpages > 1) {
                ...
                for ($i = 1; $i <= $totalpages; $i++) {
                    if ($i > 3 && $i < $totalpages) {
                        ...
                        $topic_page_jump .= '[<a href="' . $topic_obj->getURL_JUMPBOX((($i - 1) * $xoopsModuleConfig['posts_per_page']), 0) . '">' . $i . '</a>]';
                        $topic_page_jump_icon = "<a href='" . $topic_obj->getURL_JUMPBOX((($i - 1) * $xoopsModuleConfig['posts_per_page']), 0) . "'>" . forum_displayImage($xforumImage['docicon']) . "</a>";
                    }
                }
            }
            else {
            	$topic_page_jump_icon = '&nbsp;';//"<a href='" . $topic_obj->getURL_JUMPBOX(0, $myrow['post_id']) . "'>" . forum_displayImage($xforumImage['docicon']) . "</a>";
        	}
