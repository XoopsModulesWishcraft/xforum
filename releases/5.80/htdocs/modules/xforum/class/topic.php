...
class Topic extends XoopsObject 
{
    ...
    function getSUBPath()
    {
		...
		if (!isset($subpath[$this->getVar('forum_id')])) {
			...
			$forum = $forum_handler->get($this->getVar('forum_id'));
    ...
    {	
		if ($this->_ModConfig['htaccess']) {
    		return $this->domain() . '/' . $this->_ModConfig['baseurl'] . $this->getSUBPath() . '/archive,' . $this->getVar('forum_id') . ',' . $this->getVar('topic_id') . $this->_ModConfig['endofurl'];
    	} else {
    		return $this->domain() . '/modules/xforum/archive.php?forum='.$this->getVar('forum_id').'&topic_id='.$this->getVar('topic_id');
    	}
    }
    ...
    function getURL($post_id=0) {
    	...
    	$viewmode = !empty($_GET['viewmode']) ? $_GET['viewmode'] : '';
    	
    	if ($post_id>0||($start>0&&!isset($_GET['sortname']))) {
    		if ($start>0) {
	    		if ($this->_ModConfig['htaccess']) {
		    		return $this->domain() . '/' . $this->_ModConfig['baseurl'] . $this->getSUBPath() . '/' . xoops_sef($this->getVar('topic_title')) . '/' . $this->getVar('forum_id') . ',' . $this->getVar('topic_id')  . ",0,$start,$move,$type,$order" . $this->_ModConfig['endofurl'];
		    	} else {
		    		return $this->domain().'/modules/xforum/'.'viewtopic.php?forum=' . $this->getVar('forum_id') . '&amp;topic_id=' . $this->getVar('topic_id') . "&amp;start=$start&amp;move=$move&amp;mode=$mode&amp;type=$type&amp;order=$order";
		    	}
    		} elseif ($post_id>0) {
    			if ($this->_ModConfig['htaccess']) {
		    		return $this->domain() . '/' . $this->_ModConfig['baseurl'] . $this->getSUBPath() . '/' . xoops_sef($this->getVar('topic_title')) . '/' . $this->getVar('forum_id') . ',' . $this->getVar('topic_id')  . ",$post_id,$start,$move,$type,$order" . $this->_ModConfig['endofurl'] . '#forumpost' . $post_id;
		    	} else {
		    		return $this->domain().'/modules/xforum/'.'viewtopic.php?forum=' . $this->getVar('forum_id') . '&amp;topic_id=' . $this->getVar('topic_id') . "&amp;post_id=$post_id&amp;start=$start&amp;move=$move&amp;mode=$mode&amp;type=$type&amp;order=$order".'#forumpost'.$post_id;
		    	}
    		}
    	} else {
    		if (strlen($viewmode)>0) {
    			if ($this->_ModConfig['htaccess']) {
			    	return $this->domain() . '/' . $this->_ModConfig['baseurl'] . $this->getSUBPath() . '/' . xoops_sef($this->getVar('topic_title')) . '/' . $this->getVar('forum_id') . ',' . $this->getVar('topic_id')  . ",$viewmode" . $this->_ModConfig['endofurl'];
			    } else {
			    	return $this->domain().'/modules/xforum/'.'viewtopic.php?forum=' . $this->getVar('forum_id') . '&amp;topic_id=' . $this->getVar('topic_id') . "&amp;order=$order&amp;viewmode=$viewmode";
			    }
    		} else {
	    		if ($this->_ModConfig['htaccess']) {
			    	return $this->domain() . '/' . $this->_ModConfig['baseurl'] . $this->getSUBPath() . '/' . xoops_sef($this->getVar('topic_title')) . '/' . $this->getVar('forum_id') . ',' . $this->getVar('topic_id')  . "" . $this->_ModConfig['endofurl'];
			    } else {
			    	return $this->domain().'/modules/xforum/'.'viewtopic.php?forum=' . $this->getVar('forum_id') . '&amp;topic_id=' . $this->getVar('topic_id') . "&amp;order=$order";
			    }
    		}
    	}
    }
    ...
    function getURL_JUMPBOX($start=0, $post_id=0) {
		...
		if ($post_id>0) {   		
			...
			if ($this->_ModConfig['htaccess']) {
	    		...
	    		return $this->domain().'/modules/xforum/'.'viewtopic.php?forum=' . $this->getVar('forum_id') . '&amp;topic_id=' . $this->getVar('topic_id') . "&amp;start=$start&amp;move=$move&amp;mode=$mode&amp;type=$type&amp;order=$order".'#forumpost'.$post_id;
	    	}
		}
