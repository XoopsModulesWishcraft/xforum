...
$xoops_module_header .= '<script src="'.XOOPS_URL.'/Frameworks/textsanitizer/xoops.js" type="text/javascript"></script>';
	...
	if ($xoopsModuleConfig['htaccess'])
	{
		...
		if ($topic_id!=0) {			
			$sql = "SELECT forum_id FROM ".$xoopsDB->prefix('xf_topics')." WHERE topic_id = '$topic_id'";
		...
		$url = XOOPS_URL."/forums/".urlencode(str_replace(' ', '-', $forum_name))."/".urlencode(str_replace(' ', '-', $topic_title))."/$forum_id,$topic_id,$post_id,$start,$type,$mode,$move";
