...
$xoopsLogger->activated = false;
	...
	if ($GLOBALS['xoopsModuleConfig']['htaccess']) {
		$url = XOOPS_URL.'/'.$GLOBALS['xoopsModuleConfig']['baseurl'].'/rss,'.intval($_GET["c"]).','.(isset($_GET["f"])?$_GET["f"]:0).$GLOBALS['xoopsModuleConfig']['endofurl_rss'];
...
if (!$tpl->is_cached('db:xforum_rss.html', $xoopsCachedTemplateId, $compile_id)) {
	...
	$rss->setVarRss('channel_title', $xoopsConfig['sitename'].' :: '._MD_XFORUM);
	$rss->channel_link = XOOPS_URL.'/';
	$rss->setVarRss('channel_desc', $xoopsConfig['slogan'].' :: '.$GLOBALS['xforumModule']->getInfo('description'));
