...
class Post extends XoopsObject {
    ...
    function domain() {
    	...
    	$forum_obj = $forum_handler->get($this->getVar('forum_id'));
    ...
    function getURL() {
    	...
    	if ($GLOBALS['xoopsModuleConfig']['htaccess']) {
	    	return $this->domain() . '/' . $GLOBALS['xoopsModuleConfig']['baseurl'] . $this->getSUBPath() . '/' . xoops_sef($this->topic_title()) . "/$forum_id,$topic_id,$post_id" . $GLOBALS['xoopsModuleConfig']['endofurl'] . '#forumpost' . $post_id;
	    } else {
    ...
    function showPost($isadmin)
    {
        ...
        if ($GLOBALS['xoopsModuleConfig']['tag']&&file_exists(XOOPS_ROOT_PATH."/modules/tag/include/tagbar.php")) {
