<?php
// $Id: action.topic.php,v 4.03 2008/06/05 16:23:24 wishcraft Exp $
//  ------------------------------------------------------------------------ //
...
$xforum_id = isset($_POST['forum_id']) ? intval($_POST['forum_id']) : 0;
...
switch($op){
	case "restore":
		...
		$xforums_obj =& $xforum_handler->getAll($criteria_forum);
		foreach(array_keys($xforums_obj) as $id){
			$xforum_handler->synchronization($xforums_obj[$id]);
		}
	...
	case "delete":
		...
		break;
	case "move":
		if(!empty($_POST["newforum"]) && $_POST["newforum"] != $xforum_id
			&& $xforum_handler->getPermission($_POST["newforum"], 'post')
		){
			...
			$xforum_handler->synchronization($_POST["newforum"]);
			$xforum_handler->synchronization($xforum_id);
		}else{
			$category_handler =& xoops_getmodulehandler('category', 'xforum');
		    $categories = $category_handler->getAllCats('access', true);
		    $xforums = $xforum_handler->getForumsByCategory(array_keys($categories), 'post', false);
		    ...
		    echo "<input type='hidden' name='forum_id' value='{$xforum_id}' />";
...
if(empty($xforum_id)){
	...
	redirect_header("viewforum.php?forum=$xforum_id", 2, _MD_DBUPDATED);
}
