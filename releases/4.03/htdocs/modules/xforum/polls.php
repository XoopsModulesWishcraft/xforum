<?php
// $Id: polls.php,v 4.03 2008/06/05 15:35:59 wishcraft Exp $
//  ------------------------------------------------------------------------ //
...
if ( $op == "save" ) {
	...
	if ( !empty($new_poll_id) ) {
        ...
        if ( !$result = $xoopsDB->query($sql) ) {
        	xforum_message("poll adding to topic error: ".$sql);
        }
		include_once XOOPS_ROOT_PATH.'/class/template.php';
		xoops_template_clear_module_cache($xoopsModule->getVar('mid'));
	} else {
		xforum_message($poll->getHtmlErrors());
		//exit();
	}
...
if ( $op == "edit" ) {
	...
	$author_label = new XoopsFormLabel(_MD_POLL_AUTHOR, "<a href='".XOOPS_URL."/userinfo.php?uid=".$poll->getVar("user_id")."'>".xforum_getUnameFromId($poll->getVar("user_id"), $xoopsModuleConfig['show_realname'])."</a>");
...
if ( $op == "delete_ok" ) {
	...
	if ( $poll->delete() != false ) {
        ...
        if ( !$result = $xoopsDB->query($sql) ) {
        	xforum_message("poll removal from topic error: ".$sql);
        }
	}
	redirect_header("viewtopic.php?topic_id=$topic_id",1,_MD_POLL_DBUPDATED);
	//exit();
