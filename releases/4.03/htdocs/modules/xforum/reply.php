<?php
// $Id: reply.php,v 4.03 2008/06/05 15:35:59 wishcraft Exp $
//  ------------------------------------------------------------------------ //
...
if ($xoopsModuleConfig['wol_enabled']){
    ...
    if($forumpost->getVar('uid')) {
	    $r_name =xforum_getUnameFromId( $forumpost->getVar('uid'), $xoopsModuleConfig['show_realname'] );
    }else{
    ...
    foreach ($forumpostshow as $eachpost) {
    	...
    	if($eachpost->getVar('uid')) {
	    	$p_name =xforum_getUnameFromId( $eachpost->getVar('uid'), $xoopsModuleConfig['show_realname'] );
			if (xforum_isAdmin($forum_obj, $eachpost->getVar('uid'))) $isadmin = 1;
    	}else{
