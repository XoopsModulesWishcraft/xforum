<?php
// $Id: xforum_block.php,v 4.03 2008/06/05 16:23:31 wishcraft Exp $
//  ------------------------------------------------------------------------ //
...
if(defined('xforum_BLOCK_DEFINED')) return;
define('xforum_BLOCK_DEFINED',true);
...
function b_xforum_show($options)
{
    ...
    $extra_criteria = "";
	if(!empty($options[2])) {
		$time_criteria = time() - xforum_getSinceTime($options[2]);
    ...
    if(!isset($access_forums)){
    	...
    	if(!$access_obj =& $xforum_handler->getForums(0, 'access', array('forum_id', 'cat_id', 'forum_type')) ){
	    	return null;
    	}
    ...
    $xforum_criteria = ' AND t.forum_id IN (' . implode(',', $allowed_forums) . ')';
    ...
    if (!$result) {
	    xforum_message("xforum block query error: ".$query);
        return false;
    }
	...
	$author_name = xforum_getUnameFromIds(array_keys($author), $xforumConfig['show_realname'], true);
    ...
    foreach ($rows as $arr) {
        ...
        $topic['time'] = xforum_formatTimestamp($arr['post_time']);
...
function b_xforum_topic_show($options)
{
    ...
    foreach ($rows as $arr) {
        ...
        $topic['replies'] = $arr['topic_replies'];
        $topic['views'] = $arr['topic_views'];
        $topic['time'] = xforum_formatTimestamp($arr['topic_time']);
...
function b_xforum_post_show($options)
{
    ...
    $xforum_criteria = ' AND p.forum_id IN (' . implode(',', $allowed_forums) . ')';
    ...
    foreach ($rows as $arr) {
        ...
        if($options[0]=="text"){
        	...
        	if(!empty($options[5])){
	    		$post_text = xoops_substr(xforum_html2text($post_text), 0, $options[5]);
    		}
...
function b_xforum_author_show($options)
{
    ...
    if($type=="topic"){
	    $xforum_criteria = ' AND forum_id IN (' . implode(',', $allowed_forums) . ')';
	...
	$author_name = xforum_getUnameFromIds(array_keys($author), $xforumConfig['show_realname']);
...
function b_xforum_edit($options)
{
    ...
    $form .= ">"._ALL."</option>";
    $form .= xforum_forumSelectBox($options_forum);
    /*
	$xforum_handler =& xoops_getmodulehandler('forum', 'xforum');
