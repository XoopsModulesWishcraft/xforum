<?php
// $Id: index.php,v 4.03 2008/06/05 15:35:59 wishcraft Exp $
//  ------------------------------------------------------------------------ //
...
include "header.php";
	...
	if ($xoopsModuleConfig['htaccess'])
	{
		if (strpos(' '.$_SERVER['REQUEST_URI'],"modules/")){
			...
			header( "Location: ".XOOPS_URL."/forums/".str_replace(array(XOOPS_URL."/modules/xforum/","index.php"),"",$_SERVER['REQUEST_URI']));
		}
	}
/* deal with marks */
if (isset($_GET['mark_read'])){
	...
	xforum_setRead_forum($markvalue);
...
if ($xoopsModuleConfig['rss_enable'] == 1) {
    ...
    $xoopsTpl->assign("rss_button", xforum_displayImage($xforumImage['rss'], 'RSS feed'));
}
$xoopsTpl->assign(array(
	"img_hotfolder" => xforum_displayImage($xforumImage['newposts_forum']),
	"img_folder" => xforum_displayImage($xforumImage['folder_forum']),
	"img_locked_nonewposts" => xforum_displayImage($xforumImage['locked_forum']),
	"img_locked_newposts" => xforum_displayImage($xforumImage['locked_forum_newposts']),
	'img_subforum' => xforum_displayImage($xforumImage['subforum'])));
