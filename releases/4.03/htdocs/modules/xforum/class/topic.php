...
class xforumTopicHandler extends ArtObjectHandler
{
    ...
    function approve($topic_id)
    {
        ...
        if (!$result = $this->db->queryF($sql)) {
            xforum_message("xforumTopicHandler::approve error:" . $sql);
            return false;
        }
     ...
     *		<li> 1: next </li>
     * </ul>
     * @param	integer	$xforum_id	the scope for moving
     * <ul>
     ...
     * @access public
     */
    function &getByMove($topic_id, $action, $xforum_id = 0)
    {
	    $topic = null;
	    if(!empty($action)):
        $sql = "SELECT * FROM " . $this->table.
               	" WHERE 1=1".
               	(($xforum_id>0)?" AND forum_id=".intval($xforum_id):"").
    ...
    function &getByPost($post_id)
    {
        ...
        if (!$result) {
            xforum_message("xforumTopicHandler::getByPost error:" . $sql);
            return $topic;
        }
    ...
    function &getTopPost($topic_id)
    {
        ...
        $result = $this->db->query($sql);
        if (!$result) {
            xforum_message("xforumTopicHandler::getTopPost error:" . $sql);
            return $post;
        }
        $post_handler =& xoops_getmodulehandler('post', 'xforum');
    ...
    function getTopPostId($topic_id)
    {
        ...
        $result = $this->db->query($sql);
        if (!$result) {
            xforum_message("xforumTopicHandler::getTopPostId error:" . $sql);
            return false;
        }
    ...
    function &getAllPosts(&$topic, $order = "ASC", $perpage = 10, &$start, $post_id = 0, $type = "")
    {
        ...
        if ($post_id) {
            ...
            $result = $this->db->query($sql);
	        if (!$result) {
	            xforum_message("xforumTopicHandler::getAllPosts:post-count error:" . $sql);
	            return $ret;
	        }
        ...
        if (!$result) {
            xforum_message("xforumTopicHandler::getAllPosts error:" . $sql);
            return $ret;
        }
        $post_handler = &xoops_getmodulehandler('post', 'xforum');
    ...
    function showTreeItem(&$topic, &$postArray)
    {
        ...
        $postArray['post_time'] = xforum_formatTimestamp($postArray['post_time']);
    ...
    function &getAllPosters(&$topic, $isApproved = true)
    {
        ...
        $result = $this->db->query($sql);
        if (!$result) {
            xforum_message("xforumTopicHandler::getAllPosters error:" . $sql);
            return array();
        }
        $ret = array();
        while ($myrow = $this->db->fetchArray($result)) {
    ...
    function getPermission($xforum, $topic_locked = 0, $type = "view")
    {
        ...
        if(xforum_isAdmin($xforum)) return 1;
    ...
    function synchronization($object = null, $force = true)
    {
        ...
        if ( $result = $xoopsDB->query($sql) ) {
	        ...
	        if ( !$result = $xoopsDB->queryF($sql) ) {
	            //xforum_message("Could not set top post $top_post for topic: ".$sql);
	            //return false;
	        }
	        $sql = 	"UPDATE ".$xoopsDB->prefix("xf_posts").
	        		" SET pid = ".$top_post.
	        		...
	        		" 	AND pid = 0";
	        if ( !$result = $xoopsDB->queryF($sql) ) {
	            //xforum_message("Could not set post parent ID for topic: ".$sql);
	            //return false;
	        }
	        ...
	        if ( !$result = $xoopsDB->queryF($sql) ) {
	            //xforum_message("Could not concile posts for topic: ".$sql);
	            //return false;
	        }
	        */
        }
