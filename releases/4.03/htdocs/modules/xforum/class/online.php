...
class xforumOnlineHandler
{
    ...
    function update()
    {
		...
		if(!$xoopsupdate){
			xforum_message("xforum online upate error");
		}
    ...
    function &show_online()
    {
        ...
        $online = array();
        $online['image'] = xforum_displayImage($xforumImage['whosonline']);
		$online['num_total'] = $num_total;
		$online['num_user'] = $num_user;
		$online['num_anonymous'] = $num_anonymous;
        $administrator_list = xforum_isModuleAdministrators($users_id, $GLOBALS["xoopsModule"]->getVar("mid"));
        foreach ($users_online as $uid=>$user) {
            if(!empty($administrator_list[$uid])){
                $user['level']= 2;
            }
            elseif(xforum_isModerator($this->forum_object, $uid)){
                $user['level']= 1;
            }
            else{
                $user['level']= 0;
    ...
    function write($uid, $uname, $time, $xforum, $ip, $xforumtopic)
    {
        ...
        if (!$GLOBALS["xoopsDB"]->queryF($sql)) {
	        xforum_message("can not update online info: ".$sql);
            return false;
        }
		...
		if($result = $GLOBALS["xoopsDB"]->queryF($sql)){
	        ...
	        xforum_message("clean xoops online error: ".$sql);
	        return false;
        }
