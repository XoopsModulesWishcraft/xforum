...
class Forum extends ArtObject {
    ...
    function &getModerators($asUname = false)
    {
        ...
        if(count($moderators_new)>0){
	        ...
	        $moderators_new = xforum_getUnameFromIds($moderators_new);
    ...
    function disp_forumModerators($valid_moderators = 0)
    {
		...
		include_once XOOPS_ROOT_PATH.'/modules/xforum/include/functions.php';
        $moderators = xforum_getUnameFromIds($valid_moderators, !empty($xoopsModuleConfig['show_realname']), true);
...
class xforumForumHandler extends ArtObjectHandler
{
    ...
    function &getForums($cat = 0, $permission = "", $tags = null)
    {
        ...
        $xforums =& $xforum_handler->getAll($criteria, $tags);
    ...
    function getAllTopics(&$xforum, $startdate, $start, $sortname, $sortorder, $type = '', $excerpt = 0)
    {
        ...
        while ($myrow = $this->db->fetchArray($result)) {
            ...
            if ($myrow['topic_haspoll']) {
	            if ($myrow['topic_sticky']) {
	                $topic_icon = xforum_displayImage($xforumImage['folder_sticky'], _MD_TOPICSTICKY) . '<br />' . xforum_displayImage($xforumImage['poll'], _MD_TOPICHASPOLL);
	            }else{
                	$topic_icon = xforum_displayImage($xforumImage['poll'], _MD_TOPICHASPOLL);
	            }
            }elseif($myrow['topic_sticky']) {
                $topic_icon = xforum_displayImage($xforumImage['folder_sticky'], _MD_TOPICSTICKY);
            ...
            $rating_img = xforum_displayImage($xforumImage[($rating < 1)?'blank':'rate' . $rating]);
            // ------------------------------------------------------
            // topic_page_jump
            $topic_page_jump = '';
            ...
            if ($totalpages > 1) {
                ...
                for ($i = 1; $i <= $totalpages; $i++) {
                    if ($i > 3 && $i < $totalpages) {
                        ...
                        $topic_page_jump_icon = "<a href='" . XOOPS_URL . "/modules/xforum/viewtopic.php?topic_id=" . $myrow['topic_id'] . "&amp;start=" . (($i - 1) * $xoopsModuleConfig['posts_per_page']) . "#forumpost" . $myrow['post_id'] . "'>" . xforum_displayImage($xforumImage['docicon']) . "</a>";
                    }
                }
            }
            else {
            	$topic_page_jump_icon = "<a href='" . XOOPS_URL . "/modules/xforum/viewtopic.php?topic_id=" . $myrow['topic_id'] . "#forumpost" . $myrow['post_id'] . "'>" . xforum_displayImage($xforumImage['docicon']) . "</a>";
        	}
            // ------------------------------------------------------
            // => topic array
            if (is_object($viewall_forums[$myrow['forum_id']])){
                $xforum_link = '<a href="' . XOOPS_URL . '/modules/xforum/viewforum.php?forum=' . $myrow['forum_id'] . '">' . $viewall_forums[$myrow['forum_id']]->getVar('forum_name') . '</a>';
            }else {
	            $xforum_link = '';
            }
            ...
            if( $excerpt == 0 ){
	            $topic_excerpt = "";
            }elseif( ($myrow['post_karma']>0 || $myrow['require_reply']>0) && !xforum_isAdmin($xforum) ){
	            $topic_excerpt = "";
            }else{
	            $topic_excerpt = xoops_substr(xforum_html2text($myts->displayTarea($myrow['post_text'])), 0, $excerpt);
            ...
            $topics[$myrow['topic_id']] = array(
                ...
                //'topic_folder' => xforum_displayImage($topic_folder),
                ...
                'topic_time' => xforum_formatTimestamp($myrow['topic_time']),
                'topic_last_posttime' => xforum_formatTimestamp($myrow['last_post_time']),
		...
		$posters_name =& xforum_getUnameFromIds(array_keys($posters), $xoopsModuleConfig['show_realname'], true);
        $topic_isRead = xforum_isRead("topic", $reads);
        ...
        foreach(array_keys($topics) as $id){
			...
			$topics[$id]['topic_folder'] = xforum_displayImage($topic_folder);
        ...
        if ( count($topics) > 0) {
            ...
            if($result = $this->db->query($sql)) {
                while (list($topic_id) = $this->db->fetchRow($result)) {
                    $topics[$topic_id]['attachment'] = '&nbsp;' . xforum_displayImage($xforumImage['clip'], _MD_TOPICSHASATT);
                }
            }
        }
    ...
    function getPermission($xforum, $type = "access", $checkCategory = true)
    {
        ...
        if (xforum_isAdministrator()) return true;
        ...
        if ("moderate" == $type) {
            $permission = (xforum_isModerator($xforum))?1:0;
        } else {
    ...
    function cleanOrphan()
    {
        ...
        */
        ...
        $xforum_list = $this->getList(new Criteria("parent_forum", 0, ">"));
        $this->identifierName = "forum_name";
        if($parent_forums = @array_values($xforum_list)){
	        $parent_list = $this->getIds(new Criteria("forum_id", "(".implode(", ", $parent_forums).")", "IN"));
	        foreach($xforum_list as $xforum_id => $parent_forum){
		        if(in_array($parent_forum, $parent_list)) continue;
		        $xforum_obj =& $this->get($xforum_id);
		        $this->delete($xforum_obj);
		        unset($xforum_obj);
	        }
        }
		endif;
    ...
    function &display(&$xforums_obj)
    {
		...
		$reads = array();
		foreach (array_keys($xforums_obj) as $id) {
			$xforum_obj =& $xforums_obj[$id];
			if(!$xforum_obj->getVar("forum_last_post_id")) continue;
			if(!$post_obj =& $posts_obj[$xforum_obj->getVar("forum_last_post_id")]) {
				$xforum_obj->assignVar("forum_last_post_id", 0);
				continue;
			}
			$users[] = $post_obj->getVar("uid");
			if($moderators = $xforum_obj->getModerators()){
		...
		$xforum_isread = xforum_isRead("forum", $reads);
		$users_linked = xforum_getUnameFromIds(array_unique($users), !empty($xoopsModuleConfig['show_realname']), true);
		
		$xforums_array = array();
		foreach (array_keys($xforums_obj) as $id) {
			$xforum_obj =& $xforums_obj[$id];
			if(!$this->getPermission($xforum_obj, "access", false)) continue;
			
			$_forum_data = array();
			$_forum_data["forum_order"]		= $xforum_obj->getVar('forum_order');
			$_forum_data["forum_id"] 		= $id;
			$_forum_data["forum_cid"] 		= $xforum_obj->getVar('cat_id');
			$_forum_data["forum_name"] 		= $xforum_obj->getVar('forum_name');
			$_forum_data["forum_desc"] 		= $xforum_obj->getVar('forum_desc');
			$_forum_data["forum_posts"] 	= $xforum_obj->getVar("forum_posts");
			$_forum_data["forum_topics"] 	= $xforum_obj->getVar("forum_topics");
			$_forum_data["forum_type"] 		= $xforum_obj->getVar('forum_type');
			$xforum_moderators = array();
			$moderators = $xforum_obj->getModerators();
			foreach($moderators as $moderator){
				$xforum_moderators[] = @$users_linked[$moderator];
			}
			$_forum_data["forum_moderators"]	= implode(", ", array_filter($xforum_moderators));
		        
			if($xforum_obj->getVar("forum_last_post_id")):
			$post_obj =& $posts_obj[$xforum_obj->getVar("forum_last_post_id")];
		    ...
		    $_forum_data['forum_lastpost_time'] = xforum_formatTimestamp($post_obj->getVar('post_time'));
		    ...
		    if (empty($xforum_isread[$id])) {
		        $xforum_folder = ($xforum_obj->getVar('forum_type') == 1) ? $xforumImage['locked_forum_newposts'] : $xforumImage['newposts_forum'];
		    } else {
		        $xforum_folder = ($xforum_obj->getVar('forum_type') == 1) ? $xforumImage['locked_forum'] : $xforumImage['folder_forum'];
		    }
		    $_forum_data['forum_folder'] = xforum_displayImage($xforum_folder);
			
		    $xforums_array[$xforum_obj->getVar('parent_forum')][] = $_forum_data;
		}
