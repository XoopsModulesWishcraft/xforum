...
class xforumReadtopicHandler extends xforumReadHandler
{
    ...
    function setRead_items($status = 0, $xforum_id = 0, $uid = null)
    {
	    if(empty($this->mode)) return true;
	    
	    if($this->mode == 1) return $this->setRead_items_cookie($status, $xforum_id);
	    else return $this->setRead_items_db($status, $xforum_id, $uid);
    }
        
    function setRead_items_cookie($status, $xforum_id)
    {
	    $cookie_name = "LT";
	    $cookie_vars = xforum_getcookie($cookie_name, true);
	    
		$item_handler =& xoops_getmodulehandler('topic', 'xforum');
		$criteria =& new CriteriaCompo(new Criteria("forum_id", $xforum_id));
		...
		xforum_setcookie($cookie_name, $cookie_vars);
		return true;
    }
    
    function setRead_items_db($status, $xforum_id, $uid)
    {
	    if(empty($uid)){
		...
		$item_handler =& xoops_getmodulehandler('topic', 'xforum');
		$criteria_topic =& new CriteriaCompo(new Criteria("forum_id", $xforum_id));
		$criteria_topic->setSort("topic_last_post_id");
		$criteria_topic->setOrder("DESC");
		$criteria_topic->setLimit($this->items_per_forum);
		$criteria_sticky =& new CriteriaCompo(new Criteria("forum_id", $xforum_id));
