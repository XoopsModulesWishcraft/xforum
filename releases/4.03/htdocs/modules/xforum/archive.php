<?php
// $Id: archive.php,v 4.03 2008/06/05 15:35:59 wishcraft Exp $
//  ------------------------------------------------------------------------ //
...
function display_archive()
{
	...
	echo "<tr><td align='left'>".xforum_displayImage($xforumImage['f_open'])."&nbsp;&nbsp;<a href='".$xforumPath['url']."archive.php'>";
...
function display_archive_forums($cat_id, $parent_forum = 0, $level=0)
{
	...
	if ($res = $db->query($sql))
	{
		while (list($xforum_id, $xforum_name) = $db->fetch_row($res))
		{
			$permissions = get_forum_auth($xforum_id);
			if ($permissions['can_view'] == 0)
			{
				continue;
			}
			$name = $myts->htmlSpecialChars($xforum_name);
			for ($i = 0; $i<($level*4+4); $i++)
				echo "&nbsp;";
			echo "<a href='archive.php?forum=$xforum_id'><b>$name</b></a><br />";
			$newlevel = $level+1;
			display_archive_forums($cat_id, $xforum_id, $newlevel);
		}
	}
...
function display_forum_topics($xforum)
{
	...
	{
		echo "<br />&nbsp;&nbsp;&nbsp;".xforum_displayImage($xforumImage['f_close'])."&nbsp;&nbsp;<strong>".$myts->htmlSpecialChars($xforumdata['forum_name'])."</strong><br />";
	}
	else
	{
		$q = "select forum_name from ".$xforumTable['forums']." WHERE forum_id=".$xforumdata['parent_forum'];
		$row = $db->fetch_array($db->query($q));
		echo "<br />&nbsp;&nbsp;&nbsp;".xforum_displayImage($xforumImage['f_open'])."&nbsp;&nbsp;<a href='".$xforumPath['url']."archive.php?forum=".$xforumdata['parent_forum']."'>".$myts->htmlSpecialChars($row['forum_name'])."</a>";
		echo "<br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;".xforum_displayImage($xforumImage['f_close'])."&nbsp;&nbsp;<strong>".$myts->htmlSpecialChars($xforumdata['forum_name'])."</strong><br />";
	}
	echo "</td></tr></table><br />";
...
function display_topic($xforum, $topic_id, $content_only = 1)
{
	...
	if($xforumdata['parent_forum'] == 0)
	{
		echo "<br />&nbsp;&nbsp;&nbsp;".xforum_displayImage($xforumImage['f_open'])."&nbsp;&nbsp;<a href='archive.php?forum=$xforum'>".$myts->htmlSpecialChars($xforumdata['forum_name'])."</a>";
		echo "<br />".xforum_displayImage($xforumImage['f_content'])."&nbsp;&nbsp;<strong>".$myts->htmlSpecialChars($topicdata['topic_title'])."</strong><br />";
	}
	else
	{
		$q = "select forum_name from ".$xforumTable['forums']." WHERE forum_id=".$xforumdata['parent_forum'];
