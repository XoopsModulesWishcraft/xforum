<?php
// $Id: viewpost.php,v 4.03 2008/06/05 16:23:59 wishcraft Exp $
//  ------------------------------------------------------------------------ //
...
if(empty($xforum_id)){
	$xforums = $xforum_handler->getForums(0, "view");
	$access_forums = array_keys($xforums);
}else{
	$xforum_obj =& $xforum_handler->get($xforum_id);
	$xforums[$xforum_id] =& $xforum_obj;
	$access_forums = array($xforum_id);
}
...
if(isset($_GET['viewmode'])&&$_GET['viewmode']=="compact") xforum_setcookie("V", "compact", $xforumCookie['expire']);
...
if(!empty($xforum_id)){
	if (!$xforum_handler->getPermission($xforum_obj, "view")){
	...
	$xoops_pagetitle = $xforum_obj->getVar('forum_name'). ' - ' ._MD_VIEWALLPOSTS. ' [' . $xoopsModule->getVar('name'). ']';
...
foreach(array_keys($posts) as $id){
	...
	if($GLOBALS["xoopsModuleConfig"]['enable_permcheck']){
	    ...
	    if (!$suspension[$post->getVar('forum_id')] && $post->checkIdentity() && $post->checkTimelimit('edit_timelimit')
	    	|| $isadmin) 
	    {
	        $thread_buttons['edit']['image'] = xforum_displayImage($xforumImage['p_edit'], _EDIT);
	    ...
	    if ( (!$suspension[$post->getVar('forum_id')] && $post->checkIdentity() && $post->checkTimelimit('delete_timelimit')) 
	    	|| $isadmin )
	    {
	        $thread_buttons['delete']['image'] = xforum_displayImage($xforumImage['p_delete'], _DELETE);
	    ...
	    if (!$suspension[$post->getVar('forum_id')] && is_object($xoopsUser)) {
	        $thread_buttons['reply']['image'] = xforum_displayImage($xforumImage['p_reply'], _MD_REPLY);
	        ...
	        /*
	        $thread_buttons['quote']['image'] = xforum_displayImage($xforumImage['p_quote'], _MD_QUOTE);
    ...
    if (!$isadmin && $xoopsModuleConfig['reportmod_enabled']) {
        $thread_buttons['report']['image'] = xforum_displayImage($xforumImage['p_report'], _MD_REPORT);
    ...
    $xoopsTpl->append('posts',
    		array(
                ...
                'post_date' 	=> xforum_formatTimestamp($post->getVar('post_time')),
...
if ( $postCount > $post_perpage ) {
    ...
    $nav = new XoopsPageNav($postCount, $post_perpage, $start, "start", 'forum='.$xforum_id.'&amp;viewmode='.$viewmode.'&amp;type='.$type.'&amp;uid='.$uid.'&amp;order='.$order."&amp;mode=".$mode);
...
$all_link = "viewall.php?forum=".$xforum_id."&amp;start=$start";
$post_link = "viewpost.php?forum=".$xforum_id;
$newpost_link = "viewpost.php?forum=".$xforum_id."&amp;new=1";
$digest_link = "viewall.php?forum=".$xforum_id."&amp;start=$start&amp;type=digest";
$unreplied_link = "viewall.php?forum=".$xforum_id."&amp;start=$start&amp;type=unreplied";
$unread_link = "viewall.php?forum=".$xforum_id."&amp;start=$start&amp;type=unread";
...
if($viewmode=="compact"){
	$viewmode_options[]= array("link"=>"viewpost.php?viewmode=flat&amp;order=".$order."&amp;forum=".$xforum_id,	"title"=>_FLAT);
	if ($order == 'DESC') {
		$viewmode_options[]= array("link"=>"viewpost.php?viewmode=compact&amp;order=ASC&amp;forum=".$xforum_id,"title"=>_OLDESTFIRST);
	} else {
		$viewmode_options[]= array("link"=>"viewpost.php?viewmode=compact&amp;order=DESC&amp;forum=".$xforum_id,"title"=>_NEWESTFIRST);
	}
}else{
	$viewmode_options[]= array("link"=>"viewpost.php?viewmode=compact&amp;order=".$order."&amp;forum=".$xforum_id,	"title"=>_MD_COMPACT);
	if ($order == 'DESC') {
		$viewmode_options[]= array("link"=>"viewpost.php?viewmode=flat&amp;order=ASC&amp;forum=".$xforum_id,"title"=>_OLDESTFIRST);
	} else {
		$viewmode_options[]= array("link"=>"viewpost.php?viewmode=flat&amp;order=DESC&amp;forum=".$xforum_id,"title"=>_NEWESTFIRST);
	}
}
