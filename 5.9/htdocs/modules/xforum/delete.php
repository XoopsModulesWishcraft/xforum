...
if ( $topic_handler->getPermission($topic->getVar("forum_id"), $GLOBALS['topic_status'], 'delete')
	...
	$GLOBALS['xoops']->redirect("viewtopic.php?topic_id=$topic_id&amp;order=$order&amp;viewmode=$viewmode&amp;pid=$pid&amp;forum=$forum", 2, _MD_DELNOTALLOWED);
    exit();
}

if (!$isadmin && !$forumpost->checkTimelimit('delete_timelimit')){
	$GLOBALS['xoops']->redirect("viewtopic.php?forum=$forum&amp;topic_id=$topic_id&amp;post_id=$post_id&amp;order=$order&amp;viewmode=$viewmode&amp;pid=$pid",2,_MD_TIMEISUPDEL);
	exit();
}
...
if ( $ok ) {
	...
	*/    
    ...
    if ( $isDeleteOne ){
        $GLOBALS['xoops']->redirect("viewtopic.php?topic_id=$topic_id&amp;order=$order&amp;viewmode=$viewmode&amp;pid=$pid&amp;forum=$forum", 2, _MD_POSTDELETED);
    }else{
        $GLOBALS['xoops']->redirect("viewforum.php?forum=$forum", 2, _MD_POSTSDELETED);
    }
	exit();
