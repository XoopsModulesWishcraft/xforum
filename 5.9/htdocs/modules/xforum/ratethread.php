...
if (!$topic_handler->getPermission($topic_obj->getVar("forum_id"), $topic_obj->getVar('topic_status'), "post")
	...
	$GLOBALS['xoops']->redirect("javascript:history.go(-1);", 2, _NOPERM);
}

if (empty($rate)){
	$GLOBALS['xoops']->redirect(XOOPS_URL."/modules/xforum/viewtopic.php?topic_id=".$topic_id."&amp;forum=".$xforum."", 4, _MD_NOVOTERATE);
    exit();
}
$rate_handler = $GLOBALS['xoops']->getModuleHandler("rate", $GLOBALS['xforumModule']->getVar("dirname"));
if ($ratinguser != 0) {
    ...
    $post_handler = $GLOBALS['xoops']->getModuleHandler("post", $GLOBALS['xforumModule']->getVar("dirname"));
    if($post_handler->getCount($crit_post)){
        $GLOBALS['xoops']->redirect("viewtopic.php?topic_id=".$topic_id."&amp;forum=".$xforum."", 4, _MD_CANTVOTEOWN);
        exit();
    }
    ...
    if($rate_handler->getCount($crit_rate)){
        $GLOBALS['xoops']->redirect("viewtopic.php?topic_id=".$topic_id."&amp;forum=".$xforum."", 4, _MD_VOTEONCE);
        exit();
    }
}else{
...
$GLOBALS['xoops']->redirect("viewtopic.php?topic_id=".$topic_id."&amp;forum=".$xforum."", 2, $ratemessage);
exit();
