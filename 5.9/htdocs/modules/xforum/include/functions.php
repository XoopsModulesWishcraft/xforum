	...
	if(!$mid){
		if (!isset($forum_mid)) {
		    if(is_object($GLOBALS['xforumModule'])&& 'xforum' == $GLOBALS['xforumModule']->dirname()){
		        ...
		        $modhandler = $GLOBALS['xoops']->getHandler('module');
...
function forum_synchronization($type = "")
{
	...
	foreach($clean as $item){
		$handler = $GLOBALS['xoops']->getModuleHandler($item, "xforum");
		$handler->cleanOrphan();
		unset($handler);
	}
	...
	if(empty($type) || in_array("read", $type)):
		$read_handler = $GLOBALS['xoops']->getModuleHandler("readforum", "xforum");
		$read_handler->clearGarbage();
		$read_handler->synchronization();
		$read_handler = $GLOBALS['xoops']->getModuleHandler("readtopic", "xforum");
		$read_handler->clearGarbage();
		$read_handler->synchronization();
	endif;
	return true;
...
{
	$read_handler = $GLOBALS['xoops']->getModuleHandler("read".$type, "xforum");
...
function xforum_getUserSearchForm($action = false)
{
	...
	$xforum_handler = $GLOBALS['xoops']->getModuleHandler('profile', 'objects');
	// Get fields
	$fields = $xforum_handler->loadFields();

	$gperm_handler = $GLOBALS['xoops']->getHandler('groupperm');
	$config_handler = $GLOBALS['xoops']->getHandler('config');
	...
	$cat_handler = $GLOBALS['xoops']->getModuleHandler('forum');
...
function xforum_getUserForm($user, $profile = null, $action = false)
{
	...
	$cat_handler = $GLOBALS['xoops']->getModuleHandler('forum', 'xforum');
