...
if ( empty($forum) ) {
    ...
    $GLOBALS['xoops']->redirect("viewforum.php?forum=$forum", 2, _MD_ERRORPOST);
    exit();
} else {
	...
	if ( $topic_handler->getPermission($forum_obj, $GLOBALS['topic_status'], 'edit')
	    ...
	    $GLOBALS['xoops']->redirect("viewtopic.php?forum=".$forum_obj->getVar('forum_id')."&amp;topic_id=$topic_id&amp;post_id=$post_id&amp;order=$order&amp;viewmode=$viewmode&amp;pid=$pid",2,_MD_NORIGHTTOEDIT);
	    exit();
	}
    if(!$isadmin && !$forumpost->checkTimelimit('edit_timelimit')){
		$GLOBALS['xoops']->redirect("viewtopic.php?forum=".$forum_obj->getVar('forum_id')."&amp;topic_id=$topic_id&amp;post_id=$post_id&amp;order=$order&amp;viewmode=$viewmode&amp;pid=$pid",2,_MD_TIMEISUP);
    	exit();
	}
