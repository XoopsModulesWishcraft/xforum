...
$GLOBALS['xoops']->tpl->assign("forumindex", sprintf(_MD_XFORUMINDEX, htmlspecialchars($GLOBALS['xoopsConfig']['sitename'], ENT_QUOTES)));
...
if ( !empty($_POST['submit']) || !empty($_GET['submit']) || !empty($uname) || !empty($term)) {
    ...
    if ( !empty($search_username) ) {
        ...
        if ( !$result = $GLOBALS['xoopsDB']->query("SELECT uid FROM ".$GLOBALS['xoopsDB']->prefix("users")." WHERE uname LIKE '%$search_username%'") ) {
            $GLOBALS['xoops']->redirect('search.php',1,_MD_ERROROCCURED);
            exit();
        }
        $uid = array();
    ...
    if ( $andor != "EXACT" ) {
        ...
        if (!$uname_required && count($queries) == 0) {
            $GLOBALS['xoops']->redirect('search.php', 2, sprintf(_SR_KEYTOOSHORT, $GLOBALS['xoopsConfigSearch']['keyword_min']));
            exit();
        }
    } else {
        //$query = trim($query);
    ...
    if ( count($results) < 1 ) {
        $GLOBALS['xoops']->tpl->assign("lang_nomatch", _SR_NOMATCH);
    }
    else {
        foreach ($results as $row) {
            $GLOBALS['xoops']->tpl->append('results', array('forum_name' => $GLOBALS['myts']->htmlSpecialChars($row['forum_name']), 'forum_link' => $row['forum_link'], 'link' => $row['link'], 'title' => $row['title'], 'poster' => $row['poster'], 'post_time' => formatTimestamp($row['time'], "m")));
        }
        unset($results);
	...
	$GLOBALS['xoops']->tpl->assign("search_info", $search_info);
}

$xforumperms = $GLOBALS['xoops']->getModuleHandler('permission', 'xforum');
...
$select_forum .= '</select>';
$GLOBALS['xoops']->tpl->assign_by_ref("forum_selection_box", $select_forum);
$select_since = forum_sinceSelectBox($GLOBALS['xforumModuleConfig']['since_default']);
$GLOBALS['xoops']->tpl->assign_by_ref("since_selection_box", $select_since);

if ($GLOBALS['xoopsConfigSearch']['keyword_min'] > 0) {
	$GLOBALS['xoops']->tpl->assign("search_rule", sprintf(_SR_KEYIGNORE, $GLOBALS['xoopsConfigSearch']['keyword_min']));
}
