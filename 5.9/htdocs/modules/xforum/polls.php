...
if(!isset($module_handler)) $module_handler = $GLOBALS['xoops']->getHandler('module');
$GLOBALS['xoopsPoll'] = $module_handler->getByDirname('xoopspoll');
if(!is_object($GLOBALS['xoopsPoll']) || !$GLOBALS['xoopsPoll']->getVar('isactive')){
	$GLOBALS['xoops']->redirect("javascript:history.go(-1);", 2, _MD_POLLMODULE_ERROR);
	exit();
}
...
if (!$topic_handler->getPermission($GLOBALS['viewtopic_forum'], $GLOBALS['xforumtopic']->getVar('topic_status'), "view")){
    $GLOBALS['xoops']->redirect("viewforum.php?forum=".$GLOBALS['viewtopic_forum']->getVar('forum_id'),2,_MD_NORIGHTTOVIEW);
    exit();
}

if ($GLOBALS['xforumModuleConfig']['htaccess']&&!isset($_POST)) {
	$url = $forum_obj->getPOLLURL();
...
if($isadmin){
	$perm = true;
}elseif ($topic_handler->getPermission($GLOBALS['viewtopic_forum'], $GLOBALS['xforumtopic']->getVar('topic_status'), "addpoll")
	&& $GLOBALS['viewtopic_forum']->getVar('allow_polls') == 1
){
	if( ($op=="add" || $op=="save") && 
		!$GLOBALS['xforumtopic']->getVar("topic_haspoll") &&
		is_object($GLOBALS['xoopsUser']) && $GLOBALS['xoopsUser']->getVar("uid")==$GLOBALS['xforumtopic']->getVar("topic_poster")
	){ 
		$perm = true;
	}
	elseif(!empty($poll_id)){
		$poll = new XoopsPoll($poll_id);
		if(is_object($GLOBALS['xoopsUser']) && $GLOBALS['xoopsUser']->getVar("uid")==$poll->getVar("user_id")) $perm = true;
	}
}
if(!$perm){
    $GLOBALS['xoops']->redirect("viewtopic.php?topic_id=".$topic_id."&forum=".$GLOBALS['viewtopic_forum']->getVar('forum_id'),2,_MD_NORIGHTTOACCESS);
}
...
if ( $op == "save" ) {
	...
	if(empty($_POST['option_text'])){
		$GLOBALS['xoops']->redirect("javascript:history.go(-1);", 2, _MD_ERROROCCURED.': '._MD_POLL_POLLOPTIONS.' !');
	}
	...
	if($option_empty) $GLOBALS['xoops']->redirect("javascript:history.go(-1);", 2, _MD_ERROROCCURED.': '._MD_POLL_POLLOPTIONS.' !');
	...
	if ( !empty($_POST['end_time']) ) {
		$timezone = is_object($GLOBALS['xoopsUser'])? $GLOBALS['xoopsUser']->timezone() : null;
	...
	if ( !empty($new_poll_id) ) {
		...
		$sql = "UPDATE ".$GLOBALS['xoopsDB']->prefix("xf_topics")." SET topic_haspoll = 1, poll_id = $new_poll_id WHERE topic_id = $topic_id";
        if ( !$result = $GLOBALS['xoopsDB']->query($sql) ) {
        	forum_message("poll adding to topic error: ".$sql);
        }
		include_once XOOPS_ROOT_PATH.'/class/template.php';
		xoops_template_clear_module_cache($GLOBALS['xforumModule']->getVar('mid'));
	} else {
		forum_message($poll->getHtmlErrors());
		//exit();
	}
	$GLOBALS['xoops']->redirect("viewtopic.php?topic_id=$topic_id&forum=".$GLOBALS['viewtopic_forum']->getVar('forum_id'),1,_MD_POLL_DBUPDATED);
	//exit();
}
...
if ( $op == "edit" ) {
	...
	$author_label = new XoopsFormLabel(_MD_POLL_AUTHOR, "<a href='".XOOPS_URL."/userinfo.php?uid=".$poll->getVar("user_id")."'>".forum_getUnameFromId($poll->getVar("user_id"), $GLOBALS['xforumModuleConfig']['show_realname'])."</a>");
...
if ( $op == "update" ) {
	...
	include_once XOOPS_ROOT_PATH.'/class/template.php';
	xoops_template_clear_module_cache($GLOBALS['xforumModule']->getVar('mid'));
	$GLOBALS['xoops']->redirect("viewtopic.php?topic_id=$topic_id",1,_MD_POLL_DBUPDATED);
	//exit();
}
...
if ( $op == "savemore" ) {
	...
	include_once XOOPS_ROOT_PATH.'/class/template.php';
	xoops_template_clear_module_cache($GLOBALS['xforumModule']->getVar('mid'));
	$GLOBALS['xoops']->redirect("polls.php?op=edit&amp;poll_id=".$poll->getVar("poll_id")."&amp;topic_id=".$topic_id,1,_MD_POLL_DBUPDATED);
	//exit();
}
...
if ( $op == "delete_ok" ) {
	...
	if ( $poll->delete() != false ) {
		...
		$sql = "UPDATE ".$GLOBALS['xoopsDB']->prefix("xf_topics")." SET votes = 0, topic_haspoll = 0, poll_id = 0 WHERE topic_id = $topic_id";
        if ( !$result = $GLOBALS['xoopsDB']->query($sql) ) {
