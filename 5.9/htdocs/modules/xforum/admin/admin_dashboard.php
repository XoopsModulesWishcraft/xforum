...
// ------------------------------------------------------------------------- //
	...
	$GLOBALS['xoops']->loadLanguage('admin', 'profile');
	...
	switch ($op) {
	    case "createdir":
	        ...
	        $GLOBALS['xoops']->redirect('admin_dashboard.php', 2, $msg . ': ' . $path);
	        exit();
	        break;
	    ...
	    case "senddigest":
	        $digest_handler = $GLOBALS['xoops']->getModuleHandler('digest', 'xforum');
	        $res = $digest_handler->process(true);
	        $msg = ($res)?_AM_XFORUM_DIGEST_FAILED:_AM_XFORUM_DIGEST_SENT;
	        $GLOBALS['xoops']->redirect('admin_dashboard.php', 2, $msg);
	        exit();
	        break;
		case "saveapprovals":
			...
			$GLOBALS['xoops']->redirect('admin_dashboard.php');
			break;
	    case "postapprovals":
