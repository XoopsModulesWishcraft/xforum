...
if ( empty($post_id) || empty($op)) {
	$GLOBALS['xoops']->redirect("javascript:history.go(-1);", 2, _MD_NORIGHTTOACCESS);
    exit();
}
...
if(!empty($topic_id)){
	$GLOBALS['xoops']->redirect("viewtopic.php?topic_id=$topic_id", 2, _MD_DBUPDATED);
}elseif(!empty($forum_id)){
	$GLOBALS['xoops']->redirect("viewforum.php?forum=$forum_id", 2, _MD_DBUPDATED);
}else{
	$GLOBALS['xoops']->redirect("viewpost.php?uid=$uid", 2, _MD_DBUPDATED);
}
