...
if ( empty($forum) ) {
    ...
    $GLOBALS['xoops']->redirect("viewforum.php?forum=$forum", 2, _MD_ERRORTOPIC);
    exit();
} elseif ( empty($post_id) ) {
    $GLOBALS['xoops']->redirect(XOOPS_URL."/modules/xforum/viewtopic.php?topic_id=$topic_id&amp;order=$order&amp;viewmode=$viewmode&amp;pid=$pid", 2, _MD_ERRORPOST);
    exit();
}
...
if ( isset($_POST['submit']) ) {
	$report_handler = $GLOBALS['xoops']->getModuleHandler('report', 'xforum');
	...
	$GLOBALS['xoops']->redirect(XOOPS_URL."/modules/xforum/viewtopic.php?forum=$forum&amp;topic_id=$topic_id&amp;post_id=$post_id&amp;order=$order&amp;viewmode=$viewmode",2,$message);
    exit();
}else{
