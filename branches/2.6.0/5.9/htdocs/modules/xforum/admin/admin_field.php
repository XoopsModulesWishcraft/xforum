...
$profilefield_handler = $GLOBALS['xoops']->getModuleHandler('field');
switch( $op ) {
default:
case "list":
	...
	$GLOBALS['forum_handler'] = $GLOBALS['xoops']->getModuleHandler('forum');
	...
	$GLOBALS['xoops']->tpl->assign('forums', $categories);
	unset($categories);
	...
	ksort($categories);
	$GLOBALS['xoops']->tpl->assign('fieldcategories', $categories);
	$GLOBALS['xoops']->tpl->assign('token', $GLOBALS['xoopsSecurity']->getTokenHTML() );
...
case "edit":
	...
	if ( !$obj->getVar('field_config') && !$obj->getVar('field_show') && !$obj->getVar('field_edit')  ) { //If no configs exist
		$GLOBALS['xoops']->redirect('admin_field.php', 2, _AM_XFORUM_FIELDNOTCONFIGURABLE);
	}
	include_once('../include/functions.php');	
	$form = xforum_getFieldForm($obj);
	$form->display();
...
case "reorder":
	if ( !$GLOBALS['xoopsSecurity']->check()  ) {
		$GLOBALS['xoops']->redirect('admin_field.php', 3, implode(',', $GLOBALS['xoopsSecurity']->getErrors() ));
	}
	if ( isset($_POST['field_ids']) && count($_POST['field_ids']) > 0 ) {
		...
		if ( count($ids) > 0 ) {
			...
			$field_handler = $GLOBALS['xoops']->getModuleHandler('field');
			...
			if ( count($errors) == 0 ) {
				//no errors
				$GLOBALS['xoops']->redirect('admin_field.php', 2, sprintf(_AM_XFORUM_SAVEDSUCCESS, _AM_XFORUM_FIELDS) );
			} else {
				$GLOBALS['xoops']->redirect('admin_field.php', 3, implode('<br />', $errors) );
			}
		}
	}
	$GLOBALS['xoops']->redirect('admin_field.php', 2, sprintf(_AM_XFORUM_SAVEDSUCCESS, _AM_XFORUM_FIELDS) );
...
case "save":
	...
	if ( isset($_REQUEST['id'])  ) {
		$obj = $profilefield_handler->get($_REQUEST['id']);
		if ( !$obj->getVar('field_config') && !$obj->getVar('field_show') && !$obj->getVar('field_edit')  ) { //If no configs exist
			$GLOBALS['xoops']->redirect('admin_fields.php', 2, _AM_XFORUM_FIELDNOTCONFIGURABLE);
		}
	} else {
		$obj = $profilefield_handler->create();
	...
	if ( $profilefield_handler->insert($obj)  ) {
		$groupperm_handler = $GLOBALS['xoops']->getHandler('groupperm');
		...
		$GLOBALS['xoops']->redirect($url, 3, sprintf(_AM_XFORUM_SAVEDSUCCESS, _AM_XFORUM_FIELD) );
	}
...
case "delete":
	$obj = $profilefield_handler->get($_REQUEST['id']);
	if ( !$obj->getVar('field_config')  ) {
		$GLOBALS['xoops']->redirect('index.php', 2, _AM_XFORUM_FIELDNOTCONFIGURABLE);
	}
	if ( isset($_REQUEST['ok']) && $_REQUEST['ok'] == 1 ) {
		...
		if ( $profilefield_handler->delete($obj)  ) {
			$GLOBALS['xoops']->redirect('admin_field.php', 3, sprintf(_AM_XFORUM_DELETEDSUCCESS, _AM_XFORUM_FIELD) );
		} else {
			echo $obj->getHtmlErrors();
		}
	} else {
...
if ( isset($template_main)  ) {
	$GLOBALS['xoops']->tpl->display("admin:xforum|{$template_main}");
}
