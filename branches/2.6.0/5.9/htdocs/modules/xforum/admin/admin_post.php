...
if ( empty($forum) ) {
    $GLOBALS['xoops']->redirect("index.php", 2, _MD_ERRORFORUM);
    exit();
}
...
if ( !empty($_POST['contents_preview']) ) {
    ...
    if (!$postid ) {
    	$GLOBALS['xoops']->redirect('index.php', 2, _AM_forum_POSTNOTSAVED);
        exit();
    }
    ...
    if($approved){
        if (!$post_handler->approve($postid)) {
            $GLOBALS['xoops']->redirect("index.php", 1, _AM_forum_POSTNOTAPPROVED);
            exit();
        }
    ...
    $GLOBALS['xoops']->redirect('index.php', 2, $message);
    exit();
}
?>