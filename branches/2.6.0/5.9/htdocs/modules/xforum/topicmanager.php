...
if (!$forum_handler->getPermission($forum, 'moderate')
    ...
    $GLOBALS['xoops']->redirect("viewtopic.php?forum=$forum&amp;topic_id=$topic_id",2,_NOPERM);
    exit();
}
...
if ( isset($_POST['submit']) ) {
	...
	if('delete'==$mode){
		...
		if($newtopic>$topic_id || !is_object($newtopic_obj)){
    		$GLOBALS['xoops']->redirect("javascript:history.go(-1)", 2, _MD_ERROR);
			exit();
		}
        ...
        if ($newforum > 0) {
    		...
    		$GLOBALS['xoops']->redirect("javascript:history.go(-1)",2,_MD_ERRORFORUM);
        }
    }else{
        $sql = sprintf("UPDATE %s SET ".$action[$mode]['sql']." WHERE topic_id = %u", $GLOBALS['xoopsDB']->prefix("xf_topics"), $topic_id);
        if ( !$r = $GLOBALS['xoopsDB']->query($sql) ) {
    		$GLOBALS['xoops']->redirect("viewtopic.php?forum=$forum&amp;topic_id=$topic_id&amp;order=$order&amp;viewmode=$viewmode",2,_MD_ERROR_BACK.'<br />sql:'.$sql);
	        exit();
        }
